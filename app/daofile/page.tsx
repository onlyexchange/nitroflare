...
/** ===== Brand vibe (DaoFile) ===== */
const DF_FROM = '#058CC2';
const DF_TO = '#2D58A7';
const DF_GRAD = 'from-[#058CC2] to-[#2D58A7]';
...
const PLANS = [
  { id: 'df-31',  label: '31 Days',  days: 31,  priceUSD: 12.99, wasUSD: 19.95 },
  { id: 'df-90',  label: '90 Days',  days: 90,  priceUSD: 27.99, wasUSD: 39.95 },
  { id: 'df-365', label: '365 Days', days: 365, priceUSD: 59.99, wasUSD: 89.95 },
  { id: 'df-999', label: '999 Days', days: 999, priceUSD: 89.99, wasUSD: 129.95 },
] as const;
...
const METHOD_NEEDS_CHAIN: Record<Method, boolean> = {
  BTC: false,
  ETH: true, // ETH shows L2s; still pay to ETH mainnet address
  SOL: false,
  BNB: false,
  LTC: false,
  USDT: true,
...
export default function DaoFilePage() {
  ...
  // Deep-link: ?plan=df-90 -> select plan and scroll to checkout
  ...
  async function startPayment() {
    ...
    if (METHOD_NEEDS_CHAIN[method] && !chain) {
      setStatus('Select a network to continue.');
      return;
    }
    if (methodNeedsLivePrice(method) && !pricesUSD[method]) {
      setStatus('Could not fetch live price. Please try again.');
      return;
    }
  ...
  function demoAddress(m: Method) {
    switch (m) {
      case 'BTC':
        return 'bc1qexampledemoaddressxxxxxxxxxxxxxxxxxxxxxxxxxxx';
      case 'ETH':
        return '0xExampleDemoAddressXXXXXXXXXXXXXXXXXXXXXXXX';
      case 'SOL':
        return 'So11111111111111111111111111111111111111112';
      case 'BNB':
        return '0xExampleDemoAddressXXXXXXXXXXXXXXXXXXXXXXXX';
      case 'LTC':
        return 'ltc1qexampledemoaddressxxxxxxxxxxxxxxxxxxxxx';
      case 'USDT':
      case 'USDC':
        return '0xExampleDemoAddressXXXXXXXXXXXXXXXXXXXXXXXX';
    }
    ...
    <div className="min-h-screen bg-[#0b0b12] text-white">
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-black/30 border-b border-white/10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 group" aria-label="Only.Exchange — Home" prefetch={false}>
            <span className={`inline-grid h-8 w-8 place-items-center rounded-xl bg-gradient-to-br ${DF_GRAD} text-white ring-1 ring-white/20 shadow-sm transition-transform group-hover:scale-105`}>
          ...
          <a
            ...
            className={`hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r ${DF_GRAD}`}
          >
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 md:py-24">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-3xl">
            ...
            <h1 className="text-4xl md:text-6xl font-bold leading-tight">
              DaoFile <span className={`text-transparent bg-clip-text bg-gradient-to-r ${DF_GRAD}`}>Premium Keys</span>
            </h1>
            <p className="mt-4 text-white/80 text-lg max-w-2xl">
              Pay with crypto and get your DaoFile premium key <em>instantly</em> after confirmations.
            </p>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                ...
                className={`px-5 py-3 rounded-2xl bg-gradient-to-r ${DF_GRAD} inline-flex items-center gap-2 hover:opacity-95`}
              >
            ...
            <div className="mt-4 text-xs text-white/60">
              Sender pays network fees • 30-minute payment window
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section id="plans" className="py-14 border-t border-white/10 bg-white/5">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-white/70 mt-2">Instant delivery after 2 confirmations.</p>
          ...
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {PLANS.map((p) => (
              <motion.button
                ...
                className={`text-left rounded-2xl border ${selected.id === p.id ? 'border-white/40' : 'border-white/10'} bg-gradient-to-br from-white/10 to-transparent p-5`}
              >
      ...
      {/* Features — DaoFile highlights */}
      <section id="features" className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-bold">💎 DaoFile Premium Benefits</h2>
          <p className="text-white/70 mt-2">Up to <span className="text-white">50 GB</span> bandwidth every 3 days. No ads. No waiting.</p>

          <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Feature icon={<Ban className="h-5 w-5" />} title="No interruptions" text="No ads, no wait timers, and no captchas." />
            <Feature icon={<Zap className="h-5 w-5" />} title="Full-speed downloads" text="Max speed on every file, any time of day." />
            <Feature icon={<PlayCircle className="h-5 w-5" />} title="Unlock premium links" text="Instant access to premium-only links." />
            <Feature icon={<RotateCcw className="h-5 w-5" />} title="Direct downloads" text="Enable direct downloads from your account." />
            <Feature icon={<Gauge className="h-5 w-5" />} title="Fair-use bandwidth" text="Up to 50 GB every 3 days under host policy." />
            <Feature icon={<Mail className="h-5 w-5" />} title="Instant delivery" text="Premium key emailed after 2 confirmations." />
          </div>
        </div>
      </section>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-white/70">Buy DaoFile Premium Key — delivered to your email.</p>
              </div>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`px-4 py-2 rounded-2xl border text-sm inline-flex items-center gap-2
                      ${active ? 'border-white/40 bg-white/10' : 'border-white/10 hover:border-white/30 bg-white/5'}`}
                  >
                    ...
                    <button
                      ...
                      className={`w-full px-6 py-4 rounded-2xl inline-flex items-center justify-center gap-2 text-lg ${
                        !isEmailValid || (methodNeedsLivePrice(method) && !pricesUSD[method]) || (METHOD_NEEDS_CHAIN[method] && !chain) || generating
                          ...
                          : `bg-gradient-to-r ${DF_GRAD} hover:opacity-95 shadow-[0_0_25px_rgba(37,99,235,0.35)]`
                      }`}
                    >
                  ...
                  <div className="w-full flex items-center justify-center">
                    {qrURL() ? (
                      <img
                        ...
                        width={260}
                        height={260}
                        className="mt-2 rounded-xl border border-white/10 shadow-[0_0_35px_rgba(37,99,235,0.25)]"
                ...
                <div className="space-y-4">
                  ...
                  <ol className="space-y-3 text-white/80 text-sm">
                    <li className="flex items-center gap-3">
                      ...
                      <span>Select a DaoFile Premium plan</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center">
      ...
      {/* FAQ (colorful, link-rich, customer-focused) */}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <h2 className="text-3xl md:text-4xl font-bold">
              FAQ
              <span className={`ml-3 inline-block align-middle h-2 w-24 rounded-full bg-gradient-to-r ${DF_GRAD}`} />
            </h2>

            <div className="flex flex-wrap gap-2">
              <a
                href="/support"
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white bg-gradient-to-r ${DF_GRAD} hover:opacity-95 shadow-[0_0_18px_rgba(37,99,235,0.35)]`}
              >
          ...
          <div className="mt-8 grid lg:grid-cols-3 gap-6">
                ...
                a={
                  <>
                    ...
                    <p className="mt-2 text-white/70 text-xs">
                      Need help? <a href="/support" className={`font-semibold text-transparent bg-clip-text bg-gradient-to-r ${DF_GRAD} underline underline-offset-4 decoration-white/20`}>Contact support</a>.
                    </p>
                  </>
                }
              />
              ...
              <QA
                ...
                a={
                  <>
                    ...
                    <div className="mt-1 text-white/70 text-sm">We email the key to the address you entered at checkout.</div>
                  </>
                }
              />
              ...
              <QA
                q={
                  <span className="inline-flex items-center gap-2">
                    ...
                    How do I activate my DaoFile key?
                  </span>
                }
                a={
                  <>
                    <div className="rounded-xl border border-white/10 bg-gradient-to-br from-white/10 to-transparent p-3">
                    ...
                    <ol className="mt-3 list-decimal list-inside space-y-1">
                      <li>Log in to your DaoFile account. <a href="https://daofile.com/?op=my_account" target="_blank" rel="noopener noreferrer" className={`font-semibold text-transparent bg-clip-text bg-gradient-to-r ${DF_GRAD} underline underline-offset-4 decoration-white/20`}>Open My Account</a>.</li>
                      <li>Find the <em>Apply Premium Key</em> field.</li>
                      <li>Paste your key and activate.</li>
                    </ol>
                ...
                a={
                  <>
                    <ul className="list-disc list-inside space-y-1">
                      ...
                      <li><strong>Overpaid:</strong> <a href="/support" className={`font-semibold text-transparent bg-clip-text bg-gradient-to-r ${DF_GRAD} underline underline-offset-4 decoration-white/20`}>Contact support</a> with your TX hash—we’ll reconcile.</li>
                      <li><strong>Wrong network:</strong> For USDT/USDC, network must match. Not sure? <a href="/support" className={`font-semibold text-transparent bg-clip-text bg-gradient-to-r ${DF_GRAD} underline underline-offset-4 decoration-white/20`}>Reach out</a> ASAP.</li>
                    </ul>
                  </>
                }
              />
            ...
            <aside className="rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <ul className="mt-3 space-y-2 text-sm">
                <li>✔️ <a href="/daofile#plans" className="underline decoration-white/20 hover:decoration-white">DaoFile plans</a></li>
                <li>✔️ <a href="/daofile?plan=df-90#checkout" className="underline decoration-white/20 hover:decoration-white">90-day pack (deep link)</a></li>
              ...
              <div className={`mt-5 rounded-xl border border-white/10 bg-gradient-to-br ${DF_GRAD} bg-opacity-10 p-4`}>
                ...
                <a
                  href="/support"
                  className={`mt-3 inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold text-white bg-gradient-to-r ${DF_GRAD} hover:opacity-95`}
                >
                  <Mail className="h-4 w-4" />
                  Contact support
                </a>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 text-sm text-white/70">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center gap-2">
              <span className={`inline-flex h-6 w-6 rounded-lg bg-gradient-to-br ${DF_GRAD}`} />
...
function Stat({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      ...
      <div className={`mt-1 px-4 py-3 rounded-xl bg-white/5 border border-white/10 ${mono ? 'font-mono' : ''} text-lg`}>{value}</div>
    </div>
  );
}
...
function BG() {
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      {/* subtle blue/cyan blobs to match DaoFile gradient */}
      <div
        className="absolute -top-1/3 left-1/2 -translate-x-1/2 h-[80vh] w-[120vw] rounded-full blur-3xl"
        style={{ background: `radial-gradient(60% 60% at 50% 40%, ${DF_FROM}22, transparent 70%)` }}
      />
      <div
        className="absolute bottom-[-30vh] right-[-10vw] h-[60vh] w-[60vw] rounded-full blur-3xl"
        style={{ background: `radial-gradient(60% 60% at 50% 50%, ${DF_TO}1F, transparent 70%)` }}
      />
    </div>
  );
}
