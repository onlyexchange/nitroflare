...
export default function FilehostIndexPage({
  // If you keep PROVIDERS in this file, remove props + get from local const.
  providers = (globalThis as any).PROVIDERS as Provider[] | undefined,
}: {
  providers?: Provider[];
}) {
  // Fallback: @ts-ignore – if you declared PROVIDERS in this file, use it.
  const ALL: Provider[] = providers ?? (PROVIDERS as Provider[]);

  // UI state
  const [query, setQuery] = useState('');
  const [statusFilter, setStatusFilter] = useState<'all' | 'live' | 'soon'>(
    'all'
  );
  ...
  // Search matcher (name, slug, blurb, pack labels/bandwidth)
    ...
    return providersAlpha
      ...
      .filter((p) => {
        ...
        return (p.packs ?? []).some((pk) =>
          (pk.bandwidth ?? '').toLowerCase().includes('storage')
        );
      });
  ...
  return (
    <div className="min-h-screen bg-[#0b0b12] text-white">
      {/* Simple header (reuse your brand chip) */}
      ...
      <section className="relative">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 md:py-14">
          ...
          <p className="mt-2 text-white/70 max-w-2xl">
            Browse all supported filehosts, compare plans, and jump straight to
            checkout. Powerful filters help you find the exact key you need.
          </p>
        </div>
      </section>
          ...
          <div className="flex flex-col lg:flex-row gap-3 lg:items-end lg:justify-between">
            <div className="flex flex-col sm:flex-row gap-2 w-full lg:w-auto">
              ...
              <select
                value={statusFilter}
                onChange={(e) =>
                  setStatusFilter(e.target.value as 'all' | 'live' | 'soon')
                }
              ...
              <select
                ...
                onChange={(e) =>
                  setMaxPrice(
                    (e.target.value as any) === 'any'
                      ? 'any'
                      : (Number(e.target.value) as 10 | 25 | 50 | 100)
                  )
                }
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          {filtered.length === 0 ? (
            <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-white/70">
              No filehosts match your filters. Try clearing a filter or widening
              the price.
            </div>
          ) : (
                  ...
                  <div className="rounded-[22px] bg-black/40 border border-white/10 p-6">
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                      ...
                      <div className="flex items-center gap-4">
                        <span
                          className={`inline-grid place-items-center h-10 w-10 rounded-xl text-sm font-bold bg-gradient-to-br ${provider.vibe.mono}`}
                        >
                          {provider.monogram}
                        </span>
                        <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span
                          className={`text-[10px] tracking-wide uppercase px-2 py-1 rounded-md bg-gradient-to-r ${provider.vibe.chip}`}
                        >
      ...
      {/* Footer (use your global footer if shared) */}
      <footer className="border-t border-white/10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 text-sm text-white/70">
          ...
          <p className="mt-6 text-xs text-white/60 max-w-4xl">
            © 2025 Only.Exchange • All brand names and logos are property of
