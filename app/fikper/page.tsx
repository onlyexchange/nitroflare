...
/** Fikper — premium key checkout (blue vibe #0D47A1) */
...
const PLANS = [
  { id: 'fk-30',  label: '30 Days',  days: 30,  priceUSD: 19.95 },
  { id: 'fk-90',  label: '90 Days',  days: 90,  priceUSD: 49.95 },
  { id: 'fk-180', label: '180 Days', days: 180, priceUSD: 89.95 },
  { id: 'fk-365', label: '365 Days', days: 365, priceUSD: 129.95 },
] as const;
    ...
    <div className="min-h-screen bg-[#0b0b12] text-white">
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-black/30 border-b border-white/10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <a href="#checkout" onClick={e=>{e.preventDefault(); scrollToId('checkout')}} className="hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-[#0D47A1] to-[#1565C0]">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 md:py-24">
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{duration:0.6}} className="max-w-3xl">
            ...
            <h1 className="text-4xl md:text-6xl font-bold leading-tight">
              Fikper <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#0D47A1] via-[#1565C0] to-[#1E88E5]">Premium Keys</span>
            </h1>
            <p className="mt-4 text-white/80 text-lg max-w-2xl">
              Pay with crypto and get your Fikper premium key <em>instantly</em> after confirmations.
            </p>

            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#plans" onClick={e=>{e.preventDefault(); scrollToId('plans')}} className="px-5 py-3 rounded-2xl bg-gradient-to-r from-[#0D47A1] to-[#1E88E5] inline-flex items-center gap-2 hover:from-[#1565C0] hover:to-[#42A5F5]">
          ...
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {PLANS.map((p) => (
              <motion.button
                ...
                className={`text-left rounded-2xl border ${selected.id===p.id? 'border-sky-400/60' : 'border-white/10'} bg-gradient-to-br from-white/10 to-transparent p-5`}
              >
                <div className="flex items-center justify-between">
                  <div className="text-xl font-semibold">{p.label}</div>
                  {selected.id===p.id && <CheckCircle2 className="h-5 w-5 text-sky-300"/>}
                </div>
      ...
      <section id="features" className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Feature icon={<Zap className="h-5 w-5" />} title="No speed limits" text="Maximum download speed with no throttling." />
            <Feature icon={<ShieldCheck className="h-5 w-5" />} title="Secure HTTPS" text="Download securely over HTTPS—no snooping." />
            <Feature icon={<RotateCcw className="h-5 w-5" />} title="Resume support" text="Interrupted downloads can be resumed." />
            <Feature icon={<Ban className="h-5 w-5" />} title="No ads & no queues" text="Skip queues, captchas, and advertising." />
            <Feature icon={<PlayCircle className="h-5 w-5" />} title="Large files" text="Download files up to 100 GB." />
            <Feature icon={<Gauge className="h-5 w-5" />} title="Parallel downloads" text="Unlimited simultaneous downloads." />
          </div>
        </div>
      </section>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-white/70">Buy Fikper Premium Key direct to Email.</p>
              </div>
            </div>
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl border text-sm
                        ${active ? 'border-sky-400/60 bg-white/10'
                                 : 'border-white/10 hover:border-white/30 bg-white/5'}`}
                    >
                      {chainLabel(c)}
                    </button>
                    ...
                    <button
                      ...
                      className={`w-full px-6 py-4 rounded-2xl inline-flex items-center justify-center gap-2 text-lg
                        ${(!isEmailValid || (methodNeedsLivePrice(method) && !pricesUSD[method]) || (METHOD_NEEDS_CHAIN[method] && !chain) || generating)
                          ...
                          : 'bg-gradient-to-r from-[#0D47A1] via-[#1565C0] to-[#1E88E5] hover:from-[#1565C0] hover:to-[#42A5F5] shadow-[0_0_25px_rgba(13,71,161,0.45)]'}`}
                    >
                  ...
                  <div className="w-full flex items-center justify-center">
                    {qrURL() ? (
                      <img
                        ...
                        width={260}
                        height={260}
                        className="mt-2 rounded-xl border border-white/10 shadow-[0_0_35px_rgba(13,71,161,0.25)]"
                ...
                <div className="space-y-4">
                  ...
                  <ol className="space-y-3 text-white/80 text-sm">
                    <li className="flex items-center gap-3">
                      ...
                      <span>Select a Fikper Premium plan</span>
                    </li>
                    <li className="flex items-center gap-3">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <h2 className="text-3xl md:text-4xl font-bold">
              FAQ
              <span className="ml-3 inline-block align-middle h-2 w-24 rounded-full bg-gradient-to-r from-[#0D47A1]/70 via-[#1565C0]/60 to-[#1E88E5]/70" />
            </h2>
            <div className="flex flex-wrap gap-2">
              <a href="/support" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-[#0D47A1] via-[#1565C0] to-[#1E88E5] hover:from-[#1565C0] hover:to-[#42A5F5] shadow-[0_0_18px_rgba(13,71,161,0.4)]">
          ...
          <div className="mt-8 grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 grid md:grid-cols-2 gap-5">
              <QA
                q={<span className="inline-flex items-center gap-2"><QrCode className="h-4 w-4 opacity-90" />How do I buy a Fikper key?</span>}
                a={
                  <>
            ...
            <aside className="rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <div className="mt-3 text-sm text-white/80 space-y-2">
                ...
                <ol className="list-decimal list-inside space-y-1 mt-2">
                  <li>Log in to your Fikper account (<a href="https://fikper.com/register" target="_blank" rel="noopener noreferrer" className="underline decoration-white/20">register here</a> if needed).</li>
                  <li>Open <a href="https://fikper.com/settings" target="_blank" rel="noopener noreferrer" className="underline decoration-white/20">Settings</a>.</li>
                  <li>Enter your code in <em>Voucher Code</em> and click <em>Apply Code</em>.</li>
                </ol>
              </div>
            </aside>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 text-sm text-white/70">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center gap-2">
              <span className="inline-flex h-6 w-6 rounded-lg bg-gradient-to-br from-[#0D47A1] via-[#1565C0] to-[#1E88E5]" />
...
function QA({ q, a }: { q: React.ReactNode; a: React.ReactNode }) {
  return (
    <details className="group rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-sm hover:border-white/20 transition overflow-hidden">
      ...
      <div className="h-px w-full bg-gradient-to-r from-[#0D47A1]/40 via-[#1565C0]/30 to-[#1E88E5]/40" />
...
function BG(){
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      <div className="absolute -top-1/3 left-1/2 -translate-x-1/2 h-[80vh] w-[120vw] rounded-full bg-gradient-to-br from-[#0D47A1]/25 via-[#1565C0]/15 to-[#1E88E5]/25 blur-3xl"/>
      <div className="absolute bottom-[-30vh] right-[-10vw] h-[60vh] w-[60vw] rounded-full bg-gradient-to-br from-[#1E88E5]/15 via-[#0D47A1]/10 to-[#1565C0]/15 blur-3xl"/>
    </div>
  );
}
