...
/** ===== Shared price map (same as other pages) ===== */
const COINGECKO_IDS = {
  BTC:  'bitcoin',
  ETH:  'ethereum',
  SOL:  'solana',
...
/** ===== Alfafile plans ===== */
const PLANS = [
  { id: 'af-30',  label: '30 Days',  days: 30,  priceUSD: 14.99, wasUSD: 14.99,  bandwidth: '1 TB bandwidth • 1 TB storage' },
  { id: 'af-90',  label: '90 Days',  days: 90,  priceUSD: 39.99, wasUSD: 39.99,  bandwidth: '4 TB bandwidth • 3 TB storage' },
  { id: 'af-180', label: '180 Days', days: 180, priceUSD: 49.99, wasUSD: 49.99,  bandwidth: '6 TB bandwidth • 6 TB storage' },
  { id: 'af-365', label: '365 Days', days: 365, priceUSD: 99.99, wasUSD: 99.99,  bandwidth: '12 TB bandwidth • 12 TB storage' },
] as const;
    ...
    <div className="min-h-screen bg-[#081019] text-white">
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-black/30 border-b border-white/10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          >
            <span
              className="inline-grid h-8 w-8 place-items-center rounded-xl
                         bg-gradient-to-br from-lime-400 via-sky-500 to-blue-700
          ...
          <a
            ...
            className="hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-lime-500 via-sky-500 to-blue-600"
          >
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 md:py-24">
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{duration:0.6}} className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/5 ring-1 ring-white/10 px-3 py-1 text-xs text-white/70 mb-6">
              <Zap className="h-3.5 w-3.5"/> Instant delivery • Unique address • Live price lock
            </div>
            <h1 className="text-4xl md:text-6xl font-bold leading-tight">
              Alfafile <span className="text-transparent bg-clip-text bg-gradient-to-r from-lime-400 via-sky-400 to-blue-500">Premium Keys</span>
            </h1>
            <p className="mt-4 text-white/80 text-lg max-w-2xl">
              Pay with crypto and get your Alfafile premium key <em>instantly</em> after 2 confirmations.
            </p>

            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#plans" onClick={(e)=>{e.preventDefault(); scrollToId('plans');}} className="px-5 py-3 rounded-2xl bg-gradient-to-r from-lime-500 via-sky-500 to-blue-600 inline-flex items-center gap-2 hover:from-lime-400 hover:to-blue-500">
      ...
      <section id="plans" className="py-14 border-t border-white/10 bg-white/5">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-white/70 mt-2">Full-speed downloads. Ad-free. Instant email delivery.</p>
          ...
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {PLANS.map((p) => (
              <motion.button
                ...
                className={`text-left rounded-2xl border ${selected.id===p.id? 'border-lime-400/60' : 'border-white/10'} bg-gradient-to-br from-white/10 to-transparent p-5`}
              >
                <div className="flex items-center justify-between">
                  <div className="text-xl font-semibold">{p.label}</div>
                  {selected.id===p.id && <CheckCircle2 className="h-5 w-5 text-lime-300"/>}
                </div>
      ...
      <section id="features" className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Feature icon={<Folder className="h-5 w-5" />} title="Simultaneous downloads" text="Run multiple downloads at once with unlimited sessions." />
            <Feature icon={<ShieldCheck className="h-5 w-5" />} title="Ad-free experience" text="No popups, no captchas, no queues—just clean downloads." />
            <Feature icon={<ArrowRight className="h-5 w-5" />} title="Direct links" text="Skip redirects—get straight to your files." />
            <Feature icon={<PlayCircle className="h-5 w-5" />} title="Large files" text="Upload and download files up to 50 GB." />
            <Feature icon={<Zap className="h-5 w-5" />} title="Instant delivery" text="Keys are emailed automatically after confirmations." />
            <Feature icon={<Coins className="h-5 w-5" />} title="Crypto checkout" text="BTC, ETH (+ L2), SOL, BNB, LTC, USDT, USDC supported." />
          </div>
        </div>
      </section>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-white/70">Buy Alfafile Premium Key — delivered to your email.</p>
              </div>
            </div>
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl border text-sm
                        ${active ? 'border-lime-400/60 bg-white/10'
                                 : 'border-white/10 hover:border-white/30 bg-white/5'}`}
                    >
                      {chainLabel(c)}
                    </button>
                    ...
                    <button
                      ...
                      className={`w-full px-6 py-4 rounded-2xl inline-flex items-center justify-center gap-2 text-lg
                        ${(!isEmailValid || (methodNeedsLivePrice(method) && !pricesUSD[method]) || (METHOD_NEEDS_CHAIN[method] && !chain) || generating)
                          ...
                          : "bg-gradient-to-r from-lime-500 via-sky-500 to-blue-600 hover:from-lime-400 hover:to-blue-500 shadow-[0_0_25px_rgba(34,197,94,0.35)]"}`}
                    >
                ...
                <div className="space-y-4">
                  ...
                  <ol className="space-y-3 text-white/80 text-sm">
                    <li className="flex items-center gap-3">
                      ...
                      <span>Select an Alfafile plan</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center">
                    ...
                    <li className="flex items-center gap-3">
                      ...
                      <span>Generate your unique address (30-min lock)</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center">
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-xs text-white/70">
                    Payment details and QR appear here after you generate an address.
                  </div>
                </div>
              ) : (
                // Order summary
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <h2 className="text-3xl md:text-4xl font-bold">
              FAQ
              <span className="ml-3 inline-block align-middle h-2 w-24 rounded-full bg-gradient-to-r from-lime-400/70 via-sky-500/60 to-blue-600/70" />
            </h2>
            <div className="flex flex-wrap gap-2">
              <a
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl
                           ...
                           bg-gradient-to-r from-lime-500 via-sky-500 to-blue-600
                           hover:from-lime-400 hover:to-blue-500
          ...
          <div className="mt-8 grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 grid md:grid-cols-2 gap-5">
              <QA
                q={<span className="inline-flex items-center gap-2"><Mail className="h-4 w-4 opacity-90" />When do I get my key?</span>}
                a="Instantly after the required confirmations (usually minutes). The key is sent to the email you enter at checkout."
              />
                ...
                a={
                  <>
                    <ul className="list-disc list-inside space-y-1">
                      ...
                      <li><strong>Wrong network:</strong> For USDT/USDC, the network must match.</li>
                    </ul>
                  </>
                }
              />
              ...
              <QA
                q={<span className="inline-flex items-center gap-2"><ShieldCheck className="h-4 w-4 opacity-90" />How do I activate my Alfafile key?</span>}
                a={
                  <>
                    <div className="rounded-xl border border-white/10 bg-gradient-to-br from-lime-500/10 via-sky-500/10 to-blue-600/10 p-3">
                      <div className="text-sm font-semibold">Example Premium Key:</div>
                      <div className="mt-1 inline-block rounded-md border border-white/10 bg-black/40 px-2 py-1 font-mono text-sm">3XD3-GUEL-XUPJ-H61G</div>
                    </div>
                    <ol className="mt-3 list-decimal list-inside space-y-1">
                      <li>Log in to your Alfafile account. <a href="https://alfafile.net/premium" target="_blank" rel="noopener noreferrer" className="underline decoration-white/20">Create one here</a> if needed.</li>
                      <li>Visit the <a href="https://alfafile.net/premium" target="_blank" rel="noopener noreferrer" className="underline decoration-white/20">Premium / Voucher</a> page.</li>
                      <li>Paste your Premium Key in the voucher field and redeem.</li>
                    </ol>
                  </>
                }
              />
              ...
              <QA
                q={<span className="inline-flex items-center gap-2"><RefreshCw className="h-4 w-4 opacity-90" />Can I change my plan or email?</span>}
                a="During payment, email is locked. Use “Cancel / Start Over” to edit your plan/email, then click Generate again."
              />
            </div>
            ...
            <aside className="rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <ul className="mt-3 space-y-2 text-sm">
                <li>✔️ <a href="?plan=af-90#checkout" className="underline decoration-white/20 hover:decoration-white">Jump to 90-day plan</a></li>
                <li>✔️ <a href="?plan=af-365#checkout" className="underline decoration-white/20 hover:decoration-white">Jump to 365-day plan</a></li>
              ...
              <div className="mt-5 rounded-xl border border-white/10 bg-gradient-to-br from-lime-500/10 via-sky-500/10 to-blue-600/10 p-4">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 text-sm text-white/70">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center gap-2">
              <span className="inline-flex h-6 w-6 rounded-lg bg-gradient-to-br from-lime-500 via-sky-500 to-blue-700" />
...
function QA({ q, a }: { q: React.ReactNode; a: React.ReactNode }) {
  return (
    <details className="group rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-sm hover:border-white/20 transition overflow-hidden">
      ...
      <div className="h-px w-full bg-gradient-to-r from-lime-400/40 via-sky-500/30 to-blue-600/40" />
...
function BG(){
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      <div className="absolute -top-1/3 left-1/2 -translate-x-1/2 h-[80vh] w-[120vw] rounded-full bg-gradient-to-br from-lime-500/20 via-sky-500/15 to-blue-700/25 blur-3xl"/>
      <div className="absolute bottom-[-30vh] right-[-10vw] h-[60vh] w-[60vw] rounded-full bg-gradient-to-br from-blue-700/15 via-sky-500/10 to-lime-500/15 blur-3xl"/>
    </div>
  );
}
