...
/** File.al — premium key checkout (white/silver with red highlight) */
...
type Plan = {
  ...
  priceUSD: number;
  wasUSD?: number;
  // Optional decorations for the card
  note?: string;
  kind: 'time' | 'traffic';
};
const PLANS: Plan[] = [
  // Time-based
  { id: 'fa-30',  label: '30 Days',  priceUSD: 19.95, wasUSD: 29.95, note: '30GB/day • 4TB storage', kind: 'time' },
  { id: 'fa-90',  label: '90 Days',  priceUSD: 59.95,                   note: '30GB/day • 4TB storage', kind: 'time' },
  { id: 'fa-365', label: '365 Days', priceUSD: 99.95,                   note: '30GB/day • 4TB storage', kind: 'time' },

  // Traffic packs
  { id: 'fa-25gb',  label: '25 GB Traffic',  priceUSD: 17.95, kind: 'traffic' },
  { id: 'fa-50gb',  label: '50 GB Traffic',  priceUSD: 24.95, kind: 'traffic' },
  { id: 'fa-100gb', label: '100 GB Traffic', priceUSD: 39.95, kind: 'traffic' },
];
    ...
    <div className="min-h-screen bg-[#0b0b12] text-white">
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-black/30 border-b border-white/10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 group" aria-label="Only.Exchange — Home" prefetch={false}>
            <span className="inline-grid h-8 w-8 place-items-center rounded-xl bg-gradient-to-br from-rose-500 via-red-500 to-zinc-300 text-white ring-1 ring-white/20 shadow-sm transition-transform group-hover:scale-105">
          ...
          <a href="#checkout" onClick={e=>{e.preventDefault(); scrollToId('checkout')}} className="hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-zinc-200 to-rose-500 text-black">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 md:py-24">
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{duration:0.6}} className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/5 ring-1 ring-white/10 px-3 py-1 text-xs text-white/70 mb-6">
              <Zap className="h-3.5 w-3.5"/> White-label reseller • Instant email delivery
            </div>
            <h1 className="text-4xl md:text-6xl font-bold leading-tight">
              File.al <span className="text-transparent bg-clip-text bg-gradient-to-r from-zinc-200 via-zinc-300 to-rose-400">Premium Keys</span>
            </h1>
            <p className="mt-4 text-white/80 text-lg max-w-2xl">
              Buy with crypto and get your File.al premium key <em>instantly</em> after confirmations. 4&nbsp;TB storage • 30&nbsp;GB/day • 100&nbsp;GB uploads.
            </p>

            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#plans" onClick={e=>{e.preventDefault(); scrollToId('plans')}} className="px-5 py-3 rounded-2xl bg-gradient-to-r from-zinc-300 to-rose-500 text-black inline-flex items-center gap-2 hover:from-zinc-200 hover:to-rose-400">
      ...
      <section id="plans" className="py-14 border-t border-white/10 bg-white/5">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-white/70 mt-2">Time plans and traffic packs. Instant delivery after confirmations.</p>

          {/* Time plans */}
          <div className="mt-8">
            <div className="text-sm text-white/60 mb-2">Premium (time-based)</div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {PLANS.filter(p=>p.kind==='time').map((p) => (
                <motion.button
                  key={p.id}
                  onClick={()=> handleSelectPlan(p)}
                  whileHover={{scale:1.02}}
                  className={`text-left rounded-2xl border ${selected.id===p.id? 'border-rose-400/60' : 'border-white/10'} bg-gradient-to-br from-white/10 to-transparent p-5`}
                >
                  <div className="flex items-center justify-between">
                    <div className="text-xl font-semibold">{p.label}</div>
                    {selected.id===p.id && <CheckCircle2 className="h-5 w-5 text-rose-300"/>}
                  </div>
                  <div className="mt-2 flex items-baseline gap-2">
                    {p.wasUSD !== undefined && (
                      <div className="text-sm text-white/60 line-through">${p.wasUSD.toFixed(2)}</div>
                    )}
                    <div className="text-3xl font-bold">${p.priceUSD.toFixed(2)}</div>
                  </div>
                  {p.note && <div className="mt-3 text-xs text-white/70">{p.note}</div>}
                </motion.button>
              ))}
            </div>
          </div>
          ...
          {/* Traffic packs */}
          <div className="mt-10">
            <div className="text-sm text-white/60 mb-2">Traffic packs</div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {PLANS.filter(p=>p.kind==='traffic').map((p) => (
                <motion.button
                  key={p.id}
                  onClick={()=> handleSelectPlan(p)}
                  whileHover={{scale:1.02}}
                ...
                >
                  ...
                  <div className="mt-3 text-xs text-white/70">One-time data top-up</div>
                </motion.button>
              ))}
            </div>
          </div>
      ...
      <section id="features" className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Feature icon={<Gauge className="h-5 w-5" />} title="Unlimited speed" text="Maximum speed with no waiting time." />
            <Feature icon={<ShieldCheck className="h-5 w-5" />} title="Direct & instant" text="Enable direct links for immediate starts." />
            <Feature icon={<HardDrive className="h-5 w-5" />} title="4TB storage" text="Plenty of space for your files." />
            <Feature icon={<Upload className="h-5 w-5" />} title="100GB uploads" text="Upload and download files up to 100GB." />
            <Feature icon={<Gauge className="h-5 w-5" />} title="200GB/day ready" text="Parallel downloads and accelerators supported." />
            <Feature icon={<Ban className="h-5 w-5" />} title="No ads" text="Ad-free experience, no captchas, no queues." />
          </div>
        </div>
      </section>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-white/70">Buy File.al Premium Key / Traffic Pack — emailed after confirmations.</p>
              </div>
            </div>
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl border text-sm
                        ${active ? 'border-rose-400/60 bg-white/10'
                                 : 'border-white/10 hover:border-white/30 bg-white/5'}`}
                    >
                      {chainLabel(c)}
                    </button>
                    ...
                    <button
                      ...
                      className={`w-full px-6 py-4 rounded-2xl inline-flex items-center justify-center gap-2 text-lg
                        ${(!isEmailValid || (methodNeedsLivePrice(method) && !pricesUSD[method]) || (METHOD_NEEDS_CHAIN[method] && !chain) || generating)
                          ...
                          : 'bg-gradient-to-r from-zinc-200 via-zinc-300 to-rose-500 text-black hover:from-white hover:to-rose-400 shadow-[0_0_25px_rgba(244,63,94,0.35)]'}`}
                    >
                  ...
                  <div className="w-full flex items-center justify-center">
                    {qrURL() ? (
                      <img
                        ...
                        width={260}
                        height={260}
                        className="mt-2 rounded-xl border border-white/10 shadow-[0_0_35px_rgba(244,63,94,0.25)]"
              ...
              {step !== 'pay' ? (
                <div className="space-y-4">
                  ...
                  <ol className="space-y-3 text-white/80 text-sm">
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center"><Coins className="h-4 w-4 opacity-80" /></div>
                      <span>Select a File.al plan (time or traffic)</span>
                    </li>
                    <li className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="mt-4 grid sm:grid-cols-2 gap-4">
                    <Summary label="Selection" value={selected.label}/>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <h2 className="text-3xl md:text-4xl font-bold">
              FAQ
              <span className="ml-3 inline-block align-middle h-2 w-24 rounded-full bg-gradient-to-r from-zinc-300/70 via-zinc-400/60 to-rose-400/70" />
            </h2>
            <div className="flex flex-wrap gap-2">
              <a href="/support" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-zinc-300 via-zinc-400 to-rose-500 text-black hover:from-white hover:to-rose-400 shadow-[0_0_18px_rgba(244,63,94,0.35)]">
          ...
          <div className="mt-8 grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 grid md:grid-cols-2 gap-5">
              <QA
                q={<span className="inline-flex items-center gap-2"><QrCode className="h-4 w-4 opacity-90" />How do I buy a File.al key?</span>}
                a={
                  <>
                    <ol className="list-decimal list-inside space-y-1">
                      <li>Choose a time plan or traffic pack & enter your email.</li>
                    ...
                    <p className="mt-2 text-white/70 text-xs">Need help? <a href="/support" className="underline decoration-white/20">Contact support</a>.</p>
                  </>
                }
              />
              <QA
                q={<span className="inline-flex items-center gap-2"><Mail className="h-4 w-4 opacity-90" />When do I receive my key?</span>}
                a={<>Instantly after required confirmations. Your key is emailed to the address you entered at checkout.</>}
              />
              <QA
                q={<span className="inline-flex items-center gap-2"><ShieldCheck className="h-4 w-4 opacity-90" />Which coins & networks?</span>}
                a={
                  <>
                    <div>Coins: BTC, ETH, SOL, BNB, LTC, USDT, USDC.</div>
                    <div className="mt-1">USDT/USDC: Ethereum, Solana, BNB Smart Chain. ETH supports L2s (Base, Arbitrum, Optimism, Polygon, zkSync, Linea, Scroll).</div>
                  </>
                }
              />
              <QA
                ...
                a={
                  <>
                    <ul className="list-disc list-inside space-y-1">
                      <li><strong>Underpaid:</strong> send the difference to the same address while the timer is active.</li>
                      <li><strong>Overpaid:</strong> <a href="/support" className="underline decoration-white/20">contact support</a> with your TX hash.</li>
                      <li><strong>Wrong network (USDT/USDC):</strong> must match the selected network—reach out ASAP if unsure.</li>
                    </ul>
                  </>
                }
              />
              <QA
                q={<span className="inline-flex items-center gap-2"><Coins className="h-4 w-4 opacity-90" />Where do prices come from?</span>}
                a="Live crypto pricing via our /api/price (CoinGecko source). Your amount is locked for 30 minutes after Generate."
              />
            </div>
            ...
            <aside className="rounded-2xl border border-white/10 bg-white/5 p-5">
              <div className="inline-flex items-center gap-2 text-sm font-semibold">
                ...
                Activate your File.al key
              </div>
              <div className="mt-3 text-sm text-white/80 space-y-2">
                <div>You’ll receive a Premium Key like:</div>
                <div className="inline-block rounded-md border border-white/10 bg-black/40 px-2 py-1 font-mono text-sm text-white/90">ABCD-EFGH-IJKL-MNOP</div>
                <ol className="list-decimal list-inside space-y-1 mt-2">
                  <li>Log in to your File.al account (<a href="https://file.al/" target="_blank" rel="noopener noreferrer" className="underline decoration-white/20">register here</a> if needed).</li>
                  <li>Open <em>My Account</em>.</li>
                  <li>Enter your code in <em>Voucher Code</em> and click <em>Apply Code</em> to activate Premium.</li>
                </ol>
              </div>
            </aside>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 text-sm text-white/70">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center gap-2">
              <span className="inline-flex h-6 w-6 rounded-lg bg-gradient-to-br from-zinc-300 via-zinc-400 to-rose-500" />
          ...
          <p className="mt-6 text-xs text-white/60 max-w-4xl">© 2025 Only.Exchange | All brand names and logos are property of their respective owners.</p>
        </div>
      </footer>
    </div>
  );
...
function QA({ q, a }: { q: React.ReactNode; a: React.ReactNode }) {
  return (
    <details className="group rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-sm hover:border-white/20 transition overflow-hidden">
      ...
      <div className="h-px w-full bg-gradient-to-r from-zinc-300/40 via-zinc-400/30 to-rose-400/40" />
...
function BG(){
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      <div className="absolute -top-1/3 left-1/2 -translate-x-1/2 h-[80vh] w-[120vw] rounded-full bg-gradient-to-br from-zinc-400/25 via-zinc-500/15 to-rose-400/25 blur-3xl"/>
      <div className="absolute bottom-[-30vh] right-[-10vw] h-[60vh] w-[60vw] rounded-full bg-gradient-to-br from-rose-400/15 via-zinc-400/10 to-zinc-500/15 blur-3xl"/>
    </div>
  );
}
