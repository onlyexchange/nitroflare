...
/** DDownload — premium keys with crypto checkout (blue vibe) */
...
// Plans (months mapped to ~days for UI copy)
const PLANS = [
  { id: 'dd-30',  label: '1 Month',  days: 30,  priceUSD: 14.99 },
  { id: 'dd-180', label: '6 Months', days: 180, priceUSD: 49.99 },
  { id: 'dd-365', label: '12 Months', days: 365, priceUSD: 39.99, wasUSD: 69.99 },
] as const;
    ...
    <div className="min-h-screen bg-[#0b0b12] text-white">
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-black/30 border-b border-white/10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 group" aria-label="Only.Exchange — Home" prefetch={false}>
            <span className="inline-grid h-8 w-8 place-items-center rounded-xl bg-gradient-to-br from-[#3562da] via-[#153FA6] to-[#0E2A73] text-white ring-1 ring-white/20 shadow-sm transition-transform group-hover:scale-105">
          ...
          <a href="#checkout" onClick={e=>{e.preventDefault(); scrollToId('checkout')}} className="hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-[#3562da] to-[#153FA6]">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 md:py-24">
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{duration:0.6}} className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/5 ring-1 ring-white/10 px-3 py-1 text-xs text-white/70 mb-6">
              <Zap className="h-3.5 w-3.5"/> 200 GB/day • Instant email delivery
            </div>
            <h1 className="text-4xl md:text-6xl font-bold leading-tight">
              DDownload <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#6ea0ff] via-[#3562da] to-[#153FA6]">Premium Keys</span>
            </h1>
            <p className="mt-4 text-white/80 text-lg max-w-2xl">
              Pay with crypto and get your DDownload premium key <em>instantly</em> after confirmations. No waiting, no throttling.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#plans" onClick={e=>{e.preventDefault(); scrollToId('plans')}} className="px-5 py-3 rounded-2xl bg-gradient-to-r from-[#3562da] to-[#153FA6] inline-flex items-center gap-2 hover:from-[#5880ea] hover:to-[#1e49c4]">
      ...
      <section id="plans" className="py-14 border-t border-white/10 bg-white/5">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-white/70 mt-2">Unlimited speed • 200 GB/day data volume • Instant activation</p>
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {PLANS.map((p) => (
              <motion.button
                ...
                className={`text-left rounded-2xl border ${selected.id===p.id? 'border-[#6ea0ff]/60' : 'border-white/10'} bg-gradient-to-br from-white/10 to-transparent p-5`}
              >
                <div className="flex items-center justify-between">
                  <div className="text-xl font-semibold">{p.label}</div>
                  {selected.id===p.id && <CheckCircle2 className="h-5 w-5 text-[#9ec1ff]"/>}
                </div>
                <div className="mt-2 flex items-baseline gap-2">
                  <div className="text-3xl font-bold">${p.priceUSD.toFixed(2)}</div>
                  {'wasUSD' in p && p.wasUSD && (
                    <div className="text-white/50 line-through">${p.wasUSD.toFixed(2)}</div>
                  )}
                </div>
                <div className="mt-3 text-xs text-white/70">Approx. {p.days} days of premium access</div>
              </motion.button>
            ))}
          </div>
      ...
      <section id="features" className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Feature icon={<Zap className="h-5 w-5" />} title="Unlimited speed" text="No waiting time—full speed with download manager support." />
            <Feature icon={<Gauge className="h-5 w-5" />} title="200 GB daily data" text="Enjoy 200 GB/day (≈ 6 TB/mo) with no throttling." />
            <Feature icon={<PlayCircle className="h-5 w-5" />} title="Instant starts" text="Enable direct downloads for immediate starts." />
            <Feature icon={<RotateCcw className="h-5 w-5" />} title="Parallel + resume" text="Multiple files at once, resume supported." />
            <Feature icon={<ShieldCheck className="h-5 w-5" />} title="Ad-free" text="Clean experience—no ads, no captchas." />
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-white/70">Buy DDownload Premium Key direct to Email.</p>
              </div>
            </div>
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl border text-sm
                        ${active ? 'border-[#6ea0ff]/60 bg-white/10'
                                 : 'border-white/10 hover:border-white/30 bg-white/5'}`}
                    >
                      {chainLabel(c)}
                    </button>
                    ...
                    <button
                      ...
                      className={`w-full px-6 py-4 rounded-2xl inline-flex items-center justify-center gap-2 text-lg
                        ${(!isEmailValid || (methodNeedsLivePrice(method) && !pricesUSD[method]) || (METHOD_NEEDS_CHAIN[method] && !chain) || generating)
                          ...
                          : "bg-gradient-to-r from-[#3562da] via-[#2250ce] to-[#153FA6] hover:from-[#5880ea] hover:to-[#1e49c4] shadow-[0_0_25px_rgba(53,98,218,0.35)]"}`}
                    >
                  ...
                  <div className="w-full flex items-center justify-center">
                    {qrURL() ? (
                      <img
                        ...
                        width={260}
                        height={260}
                        className="mt-2 rounded-xl border border-white/10 shadow-[0_0_35px_rgba(53,98,218,0.25)]"
                ...
                <div className="space-y-4">
                  ...
                  <ol className="space-y-3 text-white/80 text-sm">
                    <li className="flex items-center gap-3">
                      ...
                      <span>Select a DDownload Premium plan</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <h2 className="text-3xl md:text-4xl font-bold">
              FAQ
              <span className="ml-3 inline-block align-middle h-2 w-24 rounded-full bg-gradient-to-r from-[#6ea0ff]/70 via-[#3562da]/60 to-[#153FA6]/70" />
            </h2>

            <div className="flex flex-wrap gap-2">
              <a href="/support" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-[#3562da] via-[#2250ce] to-[#153FA6] hover:from-[#5880ea] hover:to-[#1e49c4] shadow-[0_0_18px_rgba(53,98,218,0.35)]">
          ...
          <div className="mt-8 grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 grid md:grid-cols-2 gap-5">
              ...
              <QA
                q={<span className="inline-flex items-center gap-2"><ShieldCheck className="h-4 w-4 opacity-90" />How do I activate it on DDownload?</span>}
                a={
                  <>
                    <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                      You’ll receive a Premium Key like: <span className="font-mono text-sm">3XD3-GUEL-XUPJ-H61G</span>
                    </div>
                    <ol className="mt-3 list-decimal list-inside space-y-1">
                      <li>Log in to your DDownload account (or <a href="https://ddownload.com/register.html" target="_blank" rel="noreferrer" className="underline decoration-white/20">register</a>).</li>
                      <li>Open your <strong>My Account</strong> page.</li>
                      <li>Find <strong>Use Voucher</strong>, paste your Premium Key, and click <strong>Apply</strong>.</li>
                    </ol>
                  </>
                }
              />
              ...
              <QA
                ...
                a={
                  <ul className="list-disc list-inside space-y-1">
                    ...
                    <li><strong>Wrong network:</strong> For USDT/USDC the network must match your selection.</li>
                  </ul>
                }
              />
            </div>
            ...
            <aside className="rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <ul className="mt-3 space-y-2 text-sm">
                <li>✔️ Unlimited speeds (download manager friendly)</li>
                <li>✔️ 200 GB/day data volume</li>
                <li>✔️ Parallel downloads + resume</li>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 text-sm text-white/70">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center gap-2">
              <span className="inline-flex h-6 w-6 rounded-lg bg-gradient-to-br from-[#3562da] via-[#153FA6] to-[#0E2A73]" />
...
function QA({ q, a }: { q: React.ReactNode; a: React.ReactNode }) {
  return (
    <details className="group rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-sm hover:border-white/20 transition overflow-hidden">
      ...
      <div className="h-px w-full bg-gradient-to-r from-[#6ea0ff]/40 via-[#3562da]/30 to-[#153FA6]/40" />
...
function BG(){
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      <div className="absolute -top-1/3 left-1/2 -translate-x-1/2 h-[80vh] w-[120vw] rounded-full bg-gradient-to-br from-[#3562da]/25 via-[#153FA6]/18 to-[#0E2A73]/25 blur-3xl"/>
      <div className="absolute bottom-[-30vh] right-[-10vw] h-[60vh] w-[60vw] rounded-full bg-gradient-to-br from-[#153FA6]/15 via-[#6ea0ff]/10 to-[#3562da]/15 blur-3xl"/>
    </div>
  );
}
