...
/** ========= Emload Premium (Only.Exchange) =========
 *  - 30/90/180/365 day plans
 *  - 1 TB storage • 35 GB/day bandwidth
 *  - Instant email delivery after confirmations
 *  - Activation: https://www.emload.com/v2/premium
 */

/* ===== Price sources (CoinGecko proxy) ===== */
const COINGECKO_IDS = {
  BTC: 'bitcoin',
  ETH: 'ethereum',
  SOL: 'solana',
...
/* ===== Plans ===== */
const PLANS = [
  { id: 'em-30',  label: '30 Days',  days: 30,  priceUSD: 13.95, wasUSD: 19.95 },
  { id: 'em-90',  label: '90 Days',  days: 90,  priceUSD: 34.95, wasUSD: 49.95 },
  { id: 'em-180', label: '180 Days', days: 180, priceUSD: 55.95, wasUSD: 79.95 },
  { id: 'em-365', label: '365 Days', days: 365, priceUSD: 83.95, wasUSD: 119.95 },
] as const;
...
const METHOD_NEEDS_CHAIN: Record<Method, boolean> = {
  BTC: false,
  ETH: true,   // ETH shows L2s; address stays mainnet
  SOL: false,
  BNB: false,
  LTC: false,
  USDT: true,
...
export default function Page() {
  ...
  function computeEndpoint(m: Method, c: Chain | null) {
    ...
    if (m === 'ETH') return '/api/next-eth-address'; // mainnet address
    ...
    <div className="min-h-screen bg-[#0b0b12] text-white">
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-black/30 border-b border-white/10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <a
            ...
            className="hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-[#035DE7] to-indigo-500"
          >
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 md:py-24">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-3xl">
            ...
            <h1 className="text-4xl md:text-6xl font-bold leading-tight">
              Emload <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#4DA0FF] via-[#2E79F6] to-[#035DE7]">Premium Keys</span>
            </h1>
            <p className="mt-4 text-white/80 text-lg max-w-2xl">
              Pay with crypto and get your Emload premium key <em>instantly</em> after confirmations.
            </p>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                ...
                className="px-5 py-3 rounded-2xl bg-gradient-to-r from-[#4DA0FF] to-[#035DE7] inline-flex items-center gap-2 hover:from-[#63AAFF] hover:to-[#1F6BFF]"
              >
      ...
      <section id="plans" className="py-14 border-t border-white/10 bg-white/5">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-white/70 mt-2">Instant delivery after confirmations. 1 TB storage • 35 GB/day bandwidth.</p>
          ...
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {PLANS.map((p) => (
              <motion.button
                ...
                className={`text-left rounded-2xl border ${selected.id === p.id ? 'border-[#4DA0FF]/60' : 'border-white/10'} bg-gradient-to-br from-white/10 to-transparent p-5`}
              >
                <div className="flex items-center justify-between">
                  <div className="text-xl font-semibold">{p.label}</div>
                  {selected.id === p.id && <CheckCircle2 className="h-5 w-5 text-[#4DA0FF]" />}
                </div>
                ...
                <div className="mt-3 text-xs text-white/70">Best for: {p.days} days of premium access</div>
              </motion.button>
            ))}
          </div>
      ...
      {/* Features — Emload highlights */}
      <section id="features" className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-bold">💎 Premium Plan Highlights</h2>

          <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Feature icon={<HardDrive className="h-5 w-5" />} title="1 TB storage included" text="Keep your files safe with generous premium storage." />
            <Feature icon={<Gauge className="h-5 w-5" />} title="35 GB / day" text="Daily bandwidth that resets every 24 hours." />
            <Feature icon={<Zap className="h-5 w-5" />} title="Max speed" text="High-speed downloads with no throttling." />
            <Feature icon={<Ban className="h-5 w-5" />} title="Ad-free • No captchas" text="Skip interruptions, queues, and wait timers." />
            <Feature icon={<RotateCcw className="h-5 w-5" />} title="Resume support" text="Pause and resume large downloads seamlessly." />
            <Feature icon={<Mail className="h-5 w-5" />} title="Instant key delivery" text="Your premium key arrives by email after confirmations." />
          </div>
        </div>
      </section>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-white/70">Buy Emload Premium Key direct to your email.</p>
              </div>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`px-4 py-2 rounded-2xl border text-sm inline-flex items-center gap-2
                      ${active ? 'border-[#4DA0FF]/60 bg-white/10' : 'border-white/10 hover:border-white/30 bg-white/5'}`}
                  >
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      <input
                        ...
                        className={`flex-1 px-4 py-3 rounded-xl bg-white/5 border outline-none text-lg ${
                          email.length === 0 ? 'border-white/10' : isEmailValid ? 'border-emerald-400/60' : 'border-red-400/60'
                        }`}
                      />
                    </div>
                  </div>
                      ...
                      className={`w-full px-6 py-4 rounded-2xl inline-flex items-center justify-center gap-2 text-lg
                        ${
                          (!isEmailValid || (methodNeedsLivePrice(method) && !pricesUSD[method]) || (METHOD_NEEDS_CHAIN[method] && !chain) || generating)
                            ...
                            : 'bg-gradient-to-r from-[#4DA0FF] via-[#2E79F6] to-[#035DE7] hover:from-[#63AAFF] hover:to-[#1F6BFF] shadow-[0_0_25px_rgba(3,93,231,0.4)]'
                        }`}
                    >
                  ...
                  <div className="w-full flex items-center justify-center">
                    {qrURL() ? (
                      <img
                        ...
                        width={260}
                        height={260}
                        className="mt-2 rounded-xl border border-white/10 shadow-[0_0_35px_rgba(99,170,255,0.25)]"
                ...
                <div className="space-y-4">
                  ...
                  <ol className="space-y-3 text-white/80 text-sm">
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center"><Bitcoin className="h-4 w-4 opacity-80" /></div>
                      <span>Select an Emload Premium plan</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center"><Mail className="h-4 w-4 opacity-80" /></div>
                      <span>Enter your email for delivery</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center"><ShieldCheck className="h-4 w-4 opacity-80" /></div>
                      <span>Choose coin (and network for ETH / USDT / USDC)</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center"><QrCode className="h-4 w-4 opacity-80" /></div>
                      <span>Generate to lock price &amp; get your address</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center"><TimerIcon className="h-4 w-4 opacity-80" /></div>
                      <span>Send the exact amount within 30:00</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center"><Rocket className="h-4 w-4 opacity-80" /></div>
                      <span>Key emailed after <span className="text-white/90">confirmations</span></span>
                    </li>
                  </ol>
                ...
                <div>
                  ...
                  <div className="mt-6 space-y-3 text-center">
                    ...
                    <div className="grid sm:grid-cols-3 gap-3 text-xs">
                      ...
                      <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 flex items-center justify-center gap-2 text-white/70">
                        <ShieldCheck className="h-4 w-4 opacity-80" /> Confirmations required
                      </div>
                      <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 flex items-center justify-center gap-2 text-white/70">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <h2 className="text-3xl md:text-4xl font-bold">
              FAQ
              <span className="ml-3 inline-block align-middle h-2 w-24 rounded-full bg-gradient-to-r from-[#4DA0FF]/70 via-[#2E79F6]/60 to-[#035DE7]/70" />
            </h2>
            ...
            <div className="flex flex-wrap gap-2">
              <a
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white
                           bg-gradient-to-r from-[#4DA0FF] via-[#2E79F6] to-[#035DE7]
                           hover:from-[#63AAFF] hover:to-[#1F6BFF] shadow-[0_0_18px_rgba(3,93,231,0.4)]"
              >
                <Mail className="h-4 w-4" /> Contact support
              </a>
              <a
                href="#plans"
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm border border-white/15 bg-white/5 hover:border-white/30"
              >
                <Zap className="h-4 w-4" /> View Emload plans
              </a>
            </div>
          </div>
          ...
          <div className="mt-8 grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 grid md:grid-cols-2 gap-5">
                ...
                a={
                  <>
                    <ol className="list-decimal list-inside space-y-1">
                      <li>Pick a plan and enter your email.</li>
                    ...
                    <p className="mt-2 text-white/70 text-xs">Need a hand? <a href="/support" className="font-semibold text-transparent bg-clip-text bg-gradient-to-r from-[#4DA0FF] via-[#2E79F6] to-[#035DE7] underline underline-offset-4 decoration-white/20">Contact support</a>.</p>
                  </>
                }
              />
              ...
              <QA
                ...
                a={
                  <>
                    ...
                    <div className="mt-1 text-white/70 text-sm">We deliver to the email you entered at checkout.</div>
                  </>
                }
              />

              <QA
                q={<span className="inline-flex items-center gap-2"><Coins className="h-4 w-4 opacity-90" />Coins & networks?</span>}
                a={
                  <>
                    <div>BTC, ETH, SOL, BNB, LTC, USDT, USDC.</div>
                    <div className="mt-1">USDT/USDC: Ethereum, Solana, BNB Smart Chain.</div>
              ...
              <QA
                q={<span className="inline-flex items-center gap-2"><AlertTriangle className="h-4 w-4 opacity-90" />Wrong amount / network?</span>}
                a={
                  <>
                    <ul className="list-disc list-inside space-y-1">
                      <li><strong>Underpaid:</strong> send the difference to the same address before the timer expires.</li>
                      <li><strong>Overpaid:</strong> <a href="/support" className="font-semibold text-transparent bg-clip-text bg-gradient-to-r from-[#4DA0FF] via-[#2E79F6] to-[#035DE7] underline underline-offset-4 decoration-white/20">Contact support</a> with your TX hash.</li>
                      <li><strong>Wrong network:</strong> For USDT/USDC, the network must match. Unsure? <a href="/support" className="font-semibold text-transparent bg-clip-text bg-gradient-to-r from-[#4DA0FF] via-[#2E79F6] to-[#035DE7] underline underline-offset-4 decoration-white/20">Reach out</a>.</li>
                    </ul>
                  </>
                }
              />
              ...
              {/* Redeem instructions */}
              <QA
                q={<span className="inline-flex items-center gap-2"><ShieldCheck className="h-4 w-4 opacity-90" />How do I redeem my key?</span>}
                a={
                  <>
                    <div className="rounded-xl border border-white/10 bg-gradient-to-br from-[#4DA0FF]/10 via-[#2E79F6]/10 to-[#035DE7]/10 p-3">
                      <div className="text-sm font-semibold text-white/90">Example premium key:</div>
                    ...
                    <ol className="mt-3 list-decimal list-inside space-y-1">
                      <li>Log in to your Emload account. <a href="https://www.emload.com/v2/signup" target="_blank" rel="noopener noreferrer" className="underline decoration-white/20 hover:decoration-white">Register</a> if you don’t have one.</li>
                      <li>Visit the{' '}
                        <a
                          href="https://www.emload.com/v2/premium"
                          target="_blank"
                          rel="noopener noreferrer"
                          className="font-semibold text-transparent bg-clip-text bg-gradient-to-r from-[#4DA0FF] via-[#2E79F6] to-[#035DE7] underline underline-offset-4 decoration-white/20"
                        >
                          Activation Page
                        </a>.
                      </li>
                      <li>Click <em>Redeem Voucher</em>, paste your premium key, and confirm.</li>
                    </ol>
                  </>
                }
              />
              ...
              <QA
                q={<span className="inline-flex items-center gap-2"><RefreshCw className="h-4 w-4 opacity-90" />Change my pack or email?</span>}
              ...
              <QA
                ...
                a={
                  <>
                    ...
                    <div className="mt-1">Still missing? <a href="/support" className="font-semibold text-transparent bg-clip-text bg-gradient-to-r from-[#4DA0FF] via-[#2E79F6] to-[#035DE7] underline underline-offset-4 decoration-white/20">Open a ticket</a> with your order email and TX hash.</div>
                  </>
                }
              />
            </div>
            ...
            <aside className="rounded-2xl border border-white/10 bg-white/5 p-5">
              <div className="inline-flex items-center gap-2 text-sm font-semibold">
                <Zap className="h-4 w-4 opacity-90" /> Quick help
              </div>

              <ul className="mt-3 space-y-2 text-sm">
                <li>✔️ <a href="#plans" className="underline decoration-white/20 hover:decoration-white">Emload packs & price</a></li>
              ...
              <div className="mt-5 rounded-xl border border-white/10 bg-gradient-to-br from-[#4DA0FF]/10 via-[#2E79F6]/10 to-[#035DE7]/10 p-4">
                <div className="text-sm font-semibold">Still stuck?</div>
                <p className="text-white/70 text-sm mt-1">We can verify your transaction and resend keys if needed.</p>
                <a
                  href="/support"
                  className="mt-3 inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold text-white
                             bg-gradient-to-r from-[#4DA0FF] via-[#2E79F6] to-[#035DE7] hover:from-[#63AAFF] hover:to-[#1F6BFF]"
                >
                  <Mail className="h-4 w-4" /> Contact support
                </a>
              </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 text-sm text-white/70">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center gap-2">
              <span className="inline-flex h-6 w-6 rounded-lg bg-gradient-to-br from-[#4DA0FF] via-[#2E79F6] to-[#035DE7]" />
...
function QA({ q, a }: { q: ReactNode; a: ReactNode }) {
  return (
    <details className="group rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-sm hover:border-white/20 transition overflow-hidden">
      ...
      <div className="h-px w-full bg-gradient-to-r from-[#4DA0FF]/40 via-[#2E79F6]/30 to-[#035DE7]/40" />
...
function BG() {
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      <div className="absolute -top-1/3 left-1/2 -translate-x-1/2 h-[80vh] w-[120vw] rounded-full bg-gradient-to-br from-[#2E79F6]/25 via-[#035DE7]/15 to-[#2E79F6]/25 blur-3xl" />
      <div className="absolute bottom-[-30vh] right-[-10vw] h-[60vh] w-[60vw] rounded-full bg-gradient-to-br from-[#035DE7]/15 via-[#4DA0FF]/10 to-[#2E79F6]/15 blur-3xl" />
    </div>
  );
}
