...
/** ---------- Real-Debrid plans ---------- */
const PLANS = [
  { id: 'rd-30',  label: '30 Days',  days: 30,  priceUSD: 3.75,  wasUSD: 4.95 },
  { id: 'rd-90',  label: '90 Days',  days: 90,  priceUSD: 11.95, wasUSD: 14.95 },
  { id: 'rd-180', label: '180 Days', days: 180, priceUSD: 16.95, wasUSD: 18.95 },
  { id: 'rd-360', label: '360 Days', days: 360, priceUSD: 28.95, wasUSD: 34.95 },
] as const;
    ...
    <div className="min-h-screen bg-[#0b0b12] text-white">
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-black/30 border-b border-white/10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm text-white/80">
            ...
            <a href="#features" onClick={e=>{e.preventDefault(); scrollToId('features')}} className="hover:text-white">Benefits</a>
          ...
          <a href="#checkout" onClick={e=>{e.preventDefault(); scrollToId('checkout')}} className="hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-500 via-teal-500 to-sky-500">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 md:py-24">
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{duration:0.6}} className="max-w-3xl">
            ...
            <h1 className="text-4xl md:text-6xl font-bold leading-tight">
              Real-Debrid <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-teal-400 to-sky-400">Premium Keys</span>
            </h1>
            <p className="mt-4 text-white/80 text-lg max-w-2xl">
              Pay with crypto and get your Real-Debrid premium key <em>instantly</em> after confirmations.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#plans" onClick={e=>{e.preventDefault(); scrollToId('plans')}} className="px-5 py-3 rounded-2xl bg-gradient-to-r from-emerald-500 via-teal-500 to-sky-500 inline-flex items-center gap-2 hover:from-emerald-400 hover:to-sky-400">
      ...
      <section id="plans" className="py-14 border-t border-white/10 bg-white/5">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-white/70 mt-2">Instant delivery after confirmations.</p>
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {PLANS.map((p) => (
              <motion.button
                ...
                className={`text-left rounded-2xl border ${selected.id===p.id? 'border-emerald-400/60' : 'border-white/10'} bg-gradient-to-br from-white/10 to-transparent p-5`}
              >
                <div className="flex items-center justify-between">
                  <div className="text-xl font-semibold">{p.label}</div>
                  {selected.id===p.id && <CheckCircle2 className="h-5 w-5 text-emerald-300" />}
                </div>
                ...
                <div className="mt-3 text-xs text-white/70">Popular for: {p.days} days of premium access</div>
              </motion.button>
            ))}
          </div>
      ...
      <section id="features" className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-bold">💎 Real-Debrid Premium Benefits</h2>
          <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Feature icon={<Gauge className="h-5 w-5" />} title="High-speed downloads" text="Up to 2500 Mbps with parallel connections and accelerators." />
            <Feature icon={<PlayCircle className="h-5 w-5" />} title="HTML5 streaming" text="Stream directly in your browser—no plugins required." />
            <Feature icon={<Ban className="h-5 w-5" />} title="No ads or waiting" text="Skip queues, captchas, and embedded advertising." />
            <Feature icon={<Layers className="h-5 w-5" />} title="Multi-host access" text="Use one account to fetch from many supported hosters." />
            <Feature icon={<Download className="h-5 w-5" />} title="Unlimited traffic*" text="On supported hosts—subject to each provider’s policies." />
            <Feature icon={<Lock className="h-5 w-5" />} title="AES secure downloads" text="Extra protection for supported links and plugins." />
          </div>
        </div>
      </section>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-white/70">Buy Real-Debrid Premium Key delivered to your email.</p>
              </div>
            </div>
              ...
              {METHODS.map(m=>{
                ...
                return (
                  <button
                    key={m.id}
                    onClick={()=>{
                      if (method!==m.id){ resetPayment(); setMethod(m.id as Method); setChain(null); }
                    }}
                    className={`px-4 py-2 rounded-2xl border text-sm inline-flex items-center gap-2
                      ${active ? 'border-emerald-400/60 bg-white/10' : 'border-white/10 hover:border-white/30 bg-white/5'}`}
                  >
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid sm:grid-cols-1 gap-3">
                    <button
                      ...
                      disabled={!isEmailValid || (methodNeedsLivePrice(method) && !pricesUSD[method]) || (METHOD_NEEDS_CHAIN[method] && !chain) || generating}
                      className={`w-full px-6 py-4 rounded-2xl inline-flex items-center justify-center gap-2 text-lg
                        ${(!isEmailValid || (methodNeedsLivePrice(method) && !pricesUSD[method]) || (METHOD_NEEDS_CHAIN[method] && !chain) || generating)
                          ...
                          : 'bg-gradient-to-r from-emerald-600 via-teal-600 to-sky-600 hover:from-emerald-500 hover:to-sky-500 shadow-[0_0_25px_rgba(45,212,191,0.4)]'}`}
                    >
                  ...
                  <div className="w-full flex items-center justify-center">
                    {qrURL() ? (
                      <img
                        ...
                        width={260}
                        height={260}
                        className="mt-2 rounded-xl border border-white/10 shadow-[0_0_35px_rgba(56,189,248,0.25)]"
              ...
              {step !== 'pay' ? (
                <div className="space-y-4">
                  ...
                  <ol className="space-y-3 text-white/80 text-sm">
                    <li className="flex items-center gap-3"><DotIcon /><span>Select a Real-Debrid plan</span></li>
                    <li className="flex items-center gap-3"><DotIcon /><span>Enter your email for delivery</span></li>
                    <li className="flex items-center gap-3"><DotIcon /><span>Choose coin (and network for ETH / USDT / USDC)</span></li>
                    <li className="flex items-center gap-3"><DotIcon /><span>Generate to lock price &amp; get your address</span></li>
                    <li className="flex items-center gap-3"><DotIcon /><span>Send the exact amount within 30:00</span></li>
                    <li className="flex items-center gap-3"><DotIcon /><span>Key emailed after <span className="text-white/90">2 confirmations</span></span></li>
                  </ol>
                ...
                <div>
                  ...
                  <div className="mt-6 space-y-3 text-center">
                    ...
                    <div className="grid sm:grid-cols-3 gap-3 text-xs">
                      ...
                      <Badge><Mail className="h-4 w-4 opacity-80" /> Instant delivery</Badge>
                    </div>
                  </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <h2 className="text-3xl md:text-4xl font-bold">
              FAQ
              <span className="ml-3 inline-block align-middle h-2 w-24 rounded-full bg-gradient-to-r from-emerald-500/70 via-teal-500/60 to-sky-500/70" />
            </h2>
            <div className="flex flex-wrap gap-2">
              <a href="/support" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-emerald-600 via-teal-600 to-sky-600 hover:from-emerald-500 hover:to-sky-500 shadow-[0_0_18px_rgba(45,212,191,0.4)]">
              ...
              <a href="#plans" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm border border-white/15 bg-white/5 hover:border-white/30">
                <Zap className="h-4 w-4" /> Buy Real-Debrid keys
              </a>
            </div>
          </div>
          ...
          <div className="mt-8 grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 grid md:grid-cols-2 gap-5">
              <QA q={<QTitle icon={<QrCode />} text="How do I buy a key?" />} a={
                <>
                  ...
                  <p className="mt-2 text-white/70 text-xs">Need help? <a href="/support" className="link-grad">Contact support</a>.</p>
                </>
              } />

              <QA q={<QTitle icon={<Coins />} text="Which coins & networks are supported?" />} a={
                <>
              ...
              <QA q={<QTitle icon={<Mail />} text="When do I receive my key?" />} a={
                <>
              ...
              <QA q={<QTitle icon={<ShieldCheck />} text="How do I redeem my key?" />} a={
                <>
                  <div className="rounded-xl border border-white/10 bg-gradient-to-br from-emerald-600/10 via-teal-600/10 to-sky-600/10 p-3">
                  ...
                  <ol className="mt-3 list-decimal list-inside space-y-1">
                    <li>Log in to your Real-Debrid account (<a href="https://real-debrid.com/" target="_blank" rel="noreferrer" className="link-grad">real-debrid.com</a>).</li>
                    <li>Open the <a href="https://real-debrid.com/voucher" target="_blank" rel="noreferrer" className="link-grad">Reseller Voucher</a> page.</li>
                    <li>Paste your Premium Key in “Reseller’s code” and apply.</li>
                  </ol>
                </>
              } />

              <QA q={<QTitle icon={<AlertTriangle />} text="Sent wrong amount or network?" />} a={
                <ul className="list-disc list-inside space-y-1">
                  <li><strong>Underpaid:</strong> send the difference to the same address before the timer expires.</li>
                  <li><strong>Overpaid:</strong> <a href="/support" className="link-grad">Contact support</a> with your TX hash—We’ll reconcile.</li>
                  <li><strong>Wrong network:</strong> For USDT/USDC, the network must match. Not sure? <a href="/support" className="link-grad">Reach out</a> ASAP.</li>
                </ul>
              } />

              <QA q={<QTitle icon={<RefreshCw />} text="Can I change my pack or email?" />} a={
                <>
            ...
            <aside className="rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <ul className="mt-3 space-y-2 text-sm">
                <li>✔️ <a href="/real-debrid?plan=rd-90#checkout" className="underline decoration-white/20 hover:decoration-white">90-day pack (deep link)</a></li>
              ...
              <div className="mt-5 rounded-xl border border-white/10 bg-gradient-to-br from-emerald-600/10 via-teal-600/10 to-sky-600/10 p-4">
                <div className="text-sm font-semibold">Still stuck?</div>
                <p className="text-white/70 text-sm mt-1">We can verify your TX and resend keys.</p>
                <a href="/support" className="mt-3 inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold text-white bg-gradient-to-r from-emerald-600 via-teal-600 to-sky-600 hover:from-emerald-500 hover:to-sky-500">
                  <Mail className="h-4 w-4" /> Contact support
                </a>
              </div>
            </aside>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 text-sm text-white/70">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center gap-2">
              <span className="inline-flex h-6 w-6 rounded-lg bg-gradient-to-br from-emerald-500 via-teal-500 to-sky-500" />
...
function QTitle({ icon, text }:{icon: React.ReactNode; text: string}) {
  return <span className="inline-flex items-center gap-2"><span className="h-4 w-4 opacity-90 inline-flex items-center">{icon}</span>{text}</span>;
}
function QA({ q, a }: { q: React.ReactNode; a: React.ReactNode }) {
  return (
    <details className="group rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-sm hover:border-white/20 transition overflow-hidden">
      ...
      <div className="h-px w-full bg-gradient-to-r from-emerald-500/40 via-teal-500/30 to-sky-500/40" />
      <div className="px-4 pb-4 pt-3 text-white/80 text-sm">{a}</div>
    </details>
  );
}
function DotIcon(){ return <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center"><Bitcoin className="h-4 w-4 opacity-80"/></div>; }
function Badge({ children }:{children: React.ReactNode}) {
  return <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 flex items-center justify-center gap-2 text-white/70">{children}</div>;
}
function BG(){
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      <div className="absolute -top-1/3 left-1/2 -translate-x-1/2 h-[80vh] w-[120vw] rounded-full bg-gradient-to-br from-emerald-600/25 via-teal-600/15 to-sky-600/25 blur-3xl"/>
      <div className="absolute bottom-[-30vh] right-[-10vw] h-[60vh] w-[60vw] rounded-full bg-gradient-to-br from-sky-600/15 via-emerald-600/10 to-teal-600/15 blur-3xl"/>
    </div>
  );
}

/** utility class for gradient links in answers */
declare global {
