...
/** ───────────────────────────────────────────────────────────────────────────
 *  SilkFiles — solid red hero, white page
 *  Spec:
 *   • Hero background: #C14E31 (solid)
 *   • All headings + buttons: #C14E31
 *   • Page background: white (very light gradient)
 *  ───────────────────────────────────────────────────────────────────────── */

const COLORS = {
  primary: '#C14E31',            // red per spec
  primaryDark: '#9F3F27',        // slightly darker red for focus/hover
  glassFrom: '#FFFFFF',
  glassTo:   '#FAFAFA',
  cardEdge:  'rgba(0,0,0,0.08)', // subtle neutral borders on white
...
// SilkFiles plans
const PLANS = [
  { id: 'sf-30',  label: '30 Days',  days: 30,  priceUSD: 19.99, wasUSD: 30.00 },
  { id: 'sf-90',  label: '90 Days',  days: 90,  priceUSD: 59.99, wasUSD: 70.00 },
  { id: 'sf-180', label: '180 Days', days: 180, priceUSD: 99.99, wasUSD: 130.00 },
  { id: 'sf-365', label: '365 Days', days: 365, priceUSD: 249.99, wasUSD: 270.00 },
] as const;
...
export default function Page(){
  ...
  return (
    ...
    >
      {/* Header */}
      <header className="sticky top-0 z-40 backdrop-blur bg-white/70 border-b" style={{ borderColor: COLORS.cardEdge }}>
      ...
      {/* Hero (solid red) */}
      <section className="relative" style={{ background: COLORS.primary }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14 md:py-20 text-white">
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{duration:0.6}} className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs mb-6 bg-white/15 ring-1 ring-white/20">
              <Zap className="h-3.5 w-3.5 text-white"/> Instant delivery • Live price lock
            </div>
            <h1 className="text-4xl md:text-6xl font-bold leading-tight">
              SilkFiles.com Premium Keys
            </h1>
            <p className="mt-4 text-white/85 text-lg max-w-2xl">
              Super-fast, direct downloads with no waiting or captchas. Buy with crypto and get your key instantly after confirmations.
            </p>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                ...
                className="px-5 py-3 rounded-2xl inline-flex items-center gap-2 text-[color:var(--ink)]"
      ...
      <section id="plans" className="py-14 border-t" style={{ borderColor: COLORS.cardEdge }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-bold" style={{ color: COLORS.primary }}>Choose your plan</h2>
          <p className="text-[color:var(--ink)]/70 mt-2">Instant email delivery after confirmations.</p>
              ...
              <motion.button
                ...
                style={{
                  ...
                  boxShadow: selected.id===p.id
                    ? `0 0 0 2px ${COLORS.primary} inset, 0 8px 24px rgba(193,78,49,0.15)`
                    : '0 8px 24px rgba(17,24,39,0.06)',
                }}
              >
                <div className="flex items-center justify-between">
                  <div className="text-xs uppercase tracking-wide" style={{ color: COLORS.primary }}>
                    Premium
                  </div>
                  {selected.id===p.id && <CheckCircle2 className="h-5 w-5" style={{ color: COLORS.success }}/>}
                </div>
                <div className="mt-1 text-lg font-semibold text-[color:var(--ink)]">{p.label}</div>
      ...
      <section id="features" className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-bold" style={{ color: COLORS.primary }}>
            💎 Why Go Premium with SilkFiles.com?
          </h2>

          <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Feature icon={<Zap className="h-5 w-5" />} title="Maximum download speed" text="No throttling, no delays—just fast downloads." />
            <Feature icon={<ShieldCheck className="h-5 w-5" />} title="No waiting or captchas" text="Skip queues and captchas entirely." />
            <Feature icon={<HardDrive className="h-5 w-5" />} title="1 TB secure storage" text="Keep your files in the cloud, safely." />
            <Feature icon={<Gauge className="h-5 w-5" />} title="Direct downloads" text="Start downloads instantly from your account." />
            <Feature icon={<Coins className="h-5 w-5" />} title="Safe & encrypted" text="HTTPS and secure delivery end-to-end." />
            <Feature icon={<Mail className="h-5 w-5" />} title="Instant email delivery" text="Your key arrives right after confirmations." />
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                <h3 className="text-4xl md:text-5xl font-extrabold tracking-tight" style={{ color: COLORS.primary }}>Checkout</h3>
                <p className="mt-2 text-[color:var(--ink)]/70">Buy SilkFiles.com Premium key direct to Email.</p>
              </div>
            </div>
            ...
            <div className="mt-10 grid lg:grid-cols-2 gap-8">
              ...
              {step !== 'pay' ? (
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      <div className="flex-1 px-4 py-3 rounded-xl bg-white border text-lg"
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      <input
                        ...
                        className="flex-1 px-4 py-3 rounded-xl bg-white border outline-none text-lg"
                        style={{
                          borderColor:
                            email.length === 0
                              ? COLORS.cardEdge
                              : isEmailValid ? 'rgba(22,163,74,0.55)' : 'rgba(239,68,68,0.5)'
                        }}
                      />
                    </div>
                  </div>
                ...
                <div className="space-y-6">
                  <h4 className="text-2xl font-semibold flex items-center gap-2" style={{ color: COLORS.primary }}>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="mt-1 flex gap-2">
                        <input
                          readOnly
                          value={lockedAmount}
                          className="w-full px-4 py-3 rounded-xl bg-white border font-mono text-lg"
                          style={{ borderColor: COLORS.cardEdge }}
                        />
                        <button onClick={()=>copy(lockedAmount)} className="px-3 rounded-xl border bg-white" style={{ borderColor: COLORS.cardEdge }} title="Copy amount">
                    ...
                    <div>
                      ...
                      <div className="mt-1 flex gap-2">
                        <input readOnly value={address} className="w-full px-4 py-3 rounded-xl bg-white border font-mono text-sm" style={{ borderColor: COLORS.cardEdge }}/>
                        <button onClick={()=>copy(address)} className="px-3 rounded-xl border bg-white" style={{ borderColor: COLORS.cardEdge }} title="Copy address">
                          <Copy className="h-4 w-4"/>
                        </button>
                      </div>
                      {METHOD_NEEDS_CHAIN[method] && chain && (
                  ...
                  <div className="w-full flex items-center justify-center">
                    {qrURL() ? (
                      <img
                        ...
                        style={{ borderColor: COLORS.cardEdge, boxShadow: '0 0 35px rgba(0,0,0,0.08)' }}
              ...
              {step !== 'pay' ? (
                <div className="space-y-4">
                  <h4 className="text-2xl font-semibold" style={{ color: COLORS.primary }}>How it works</h4>
                  <ol className="space-y-3 text-[color:var(--ink)]/80 text-sm">
                    <Step icon={<Bitcoin className="h-4 w-4" />} text="Select a Premium plan" />
                  ...
                  <div className="rounded-2xl border bg-white p-4 text-xs text-[color:var(--ink)]/70" style={{ borderColor: COLORS.cardEdge }}>
                    Payment details and QR will appear after you generate an address.
                  </div>
                </div>
              ) : (
                <div>
                  <h5 className="text-2xl font-semibold" style={{ color: COLORS.primary }}>Order Summary</h5>
                  ...
                  <div className="mt-6 grid sm:grid-cols-2 gap-3">
                    <button
                      ...
                      className="w-full px-6 py-4 rounded-2xl border bg-white"
                      style={{ borderColor: COLORS.cardEdge }}
                    >
                      Cancel / Start Over
                    </button>
      ...
      {/* FAQ (activation moved here to keep checkout aligned) */}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <a
                href="#plans"
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm border bg-white"
                style={{ borderColor: COLORS.cardEdge }}
              >
                <Zap className="h-4 w-4" />
                View SilkFiles plans
              </a>
            </div>
          </div>
          ...
          <div className="mt-8 grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 grid md:grid-cols-2 gap-5">
                ...
                a={
                  <>
                    <ol className="list-decimal list-inside space-y-1">
                      <li>Choose a <span className="font-semibold">Premium</span> plan.</li>
              ...
              />
              <QA
                q={<><Mail className="h-4 w-4 opacity-90" /> How to activate my SilkFiles key?</>}
                a={
                  <>
                    ...
                    <ol className="mt-2 list-decimal list-inside space-y-1">
                      <li>Log in to your SilkFiles account (<a className="underline" href="https://silkfiles.com/register.html" target="_blank" rel="noreferrer">create one here</a> if needed).</li>
            ...
            {/* Side panel */}
            <aside className="rounded-2xl border bg-white p-5" style={{ borderColor: COLORS.cardEdge }}>
              ...
              <ul className="mt-3 space-y-2 text-sm text-[color:var(--ink)]/80">
                <li>✔️ 1 TB cloud storage included</li>
                <li>✔️ No waiting, no captchas</li>
                <li>✔️ Keys auto-send after 2 confs</li>
              </ul>
              <div className="mt-5 rounded-xl border bg-white p-4" style={{ borderColor: COLORS.cardEdge }}>
  ...
  return (
    <li className="flex items-center gap-3">
      <div className="h-7 w-7 rounded-xl grid place-items-center bg-white"
           style={{ border: `1px solid ${COLORS.cardEdge}`, color: COLORS.primary }}>
        {icon}
      </div>
      <span>{text}</span>
    </li>
...
function Badge({ children }:{ children: React.ReactNode }){
  return (
    <div className="rounded-xl px-3 py-2 flex items-center justify-center bg-white"
         style={{ border: `1px solid ${COLORS.cardEdge}` }}>
      <ShieldCheck className="h-4 w-4 mr-2" style={{ color: COLORS.success }}/>
...
function Stat({ label, value, mono=false }:{label:string; value:string; mono?:boolean}){
  return (
    <div>
      ...
      <div className={`mt-1 px-4 py-3 rounded-xl bg-white border text-lg ${mono ? 'font-mono' : ''}`}
           style={{ borderColor: COLORS.cardEdge }}>
        {value}
      </div>
    </div>
...
function Feature({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }){
  return (
    <div className="rounded-2xl border bg-white p-5"
         style={{ borderColor: COLORS.cardEdge }}>
      <div className="inline-flex items-center gap-2" style={{ color: COLORS.primary }}>
...
function QA({ q, a }: { q: React.ReactNode; a: React.ReactNode }) {
  return (
    <details className="group rounded-2xl border bg-white hover:bg-white transition overflow-hidden"
             style={{ borderColor: COLORS.cardEdge }}>
      ...
      <div className="h-px w-full" style={{ background: COLORS.primary }} />
