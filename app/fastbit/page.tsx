...
/** ====== PRICING + COIN SETUP (shared shape with your other pages) ====== */
const COINGECKO_IDS = {
  BTC: 'bitcoin',
  ETH: 'ethereum',
  SOL: 'solana',
...
// FastBit packs
const PLANS = [
  { id: 'fb-30',  label: '1 Month',   days: 30,  priceUSD: 14.95, wasUSD: 24.95, bandwidth: '50 GB/day' },
  { id: 'fb-180', label: '3 Months',  days: 180, priceUSD: 42.95, wasUSD: 49.95,              bandwidth: '50 GB/day' },
  { id: 'fb-365', label: '1 Year',  days: 365, priceUSD: .95,   wasUSD: 99.95,            bandwidth: '50 GB/day' },
] as const;
...
/** =============================== PAGE =============================== */
export default function FastBitPage() {
  const [selected, setSelected] = useState<Plan>(PLANS[2]); // default 365 (best value)
  ...
  const brandChip = 'bg-gradient-to-r from-[#05B565] via-emerald-500 to-[#05B565]';
  const brandRing = 'from-[#39505A]/70 via-[#3f5c68]/60 to-[#223139]/70';
    ...
    <div className="min-h-screen bg-[#0b0b12] text-white">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 md:py-24">
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{duration:0.6}} className="max-w-3xl">
            ...
            <h1 className="text-4xl md:text-6xl font-bold leading-tight">
              FastBit <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#05B565] via-emerald-400 to-[#05B565]">Premium Keys</span>
            </h1>
            <p className="mt-4 text-white/80 text-lg max-w-2xl">
              Pay with crypto and get your FastBit premium key <em>instantly</em> after confirmations. 50&nbsp;GB/day, no waiting, streaming video.
            </p>
      ...
      <section id="plans" className="py-14 border-t border-white/10 bg-white/5">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-white/70 mt-2">50 GB/day • Instant delivery after 2 confirmations.</p>

          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {PLANS.map((p) => {
              const isBest = p.id === 'fb-365';
              return (
                <motion.button
                  key={p.id}
                  onClick={() => handleSelectPlan(p)}
                  whileHover={{ scale: 1.02 }}
                  className={`relative text-left rounded-2xl border ${selected.id === p.id ? 'border-[#05B565]/70' : 'border-white/10'} bg-gradient-to-br from-white/10 to-transparent p-5`}
                >
                  {isBest && (
                    <span className="absolute -top-2 -right-2 inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-semibold bg-[#05B565] text-black">
                      <Star className="h-3 w-3" /> Best Value
                    </span>
                  )}
                  <div className="flex items-center justify-between">
                    <div className="text-xl font-semibold">{p.label}</div>
                    {selected.id === p.id && <CheckCircle2 className="h-5 w-5 text-[#05B565]" />}
                  </div>
      ...
      <section id="features" className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Feature icon={<Gauge className="h-5 w-5" />} title="50 GB / Day" text="Plenty of daily traffic for heavy usage." />
            <Feature icon={<Zap className="h-5 w-5" />} title="Unlimited speed" text="No throttling — max it out." />
            <Feature icon={<Ban className="h-5 w-5" />} title="No waiting" text="Skip queues, timers, and captcha." />
            <Feature icon={<PlayCircle className="h-5 w-5" />} title="Streaming video" text="Stream supported media instantly." />
            <Feature icon={<ShieldCheck className="h-5 w-5" />} title="Stable & private" text="Exact-amount QR & address per session." />
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-white/70">Buy FastBit.cc Premium Key direct to Email.</p>
              </div>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`px-4 py-2 rounded-2xl border text-sm inline-flex items-center gap-2 ${
                      active ? 'border-[#05B565]/70 bg-white/10' : 'border-white/10 hover:border-white/30 bg-white/5'
                    }`}
                  >
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      <input
                        ...
                        className={`flex-1 px-4 py-3 rounded-xl bg-white/5 border outline-none text-lg ${
                          email.length === 0 ? 'border-white/10' : isEmailValid ? 'border-[#05B565]/70' : 'border-red-400/60'
                        }`}
                      />
                    </div>
                  </div>
                    ...
                    <button
                      ...
                      className={`w-full px-6 py-4 rounded-2xl inline-flex items-center justify-center gap-2 text-lg ${
                        (!isEmailValid || (methodNeedsLivePrice(method) && !pricesUSD[method]) || (METHOD_NEEDS_CHAIN[method] && !chain) || generating)
                          ...
                          : `shadow-[0_0_25px_rgba(5,181,101,0.35)] ${brandChip}`
                      }`}
                    >
                  ...
                  <div className="w-full flex items-center justify-center">
                    {qrURL() ? (
                      <img
                        ...
                        width={260}
                        height={260}
                        className="mt-2 rounded-xl border border-white/10 shadow-[0_0_35px_rgba(5,181,101,0.25)]"
                ...
                <div className="space-y-4">
                  ...
                  <ol className="space-y-3 text-white/80 text-sm">
                    <li className="flex items-center gap-3">
                      ...
                      <span>Select a FastBit Premium plan</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center">
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm">
                    <div className="font-semibold">Activate your FastBit Key</div>
                    ...
                    <ol className="mt-2 list-decimal list-inside text-white/70 space-y-1">
                      <li>Log in at <a href="https://fastbit.cc/" target="_blank" rel="noopener noreferrer" className="underline decoration-white/20">fastbit.cc</a></li>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <h2 className="text-3xl md:text-4xl font-bold">
              FAQ
              <span className="ml-3 inline-block align-middle h-2 w-24 rounded-full bg-gradient-to-r from-[#05B565]/70 via-emerald-500/60 to-[#05B565]/70" />
            </h2>
          ...
          <div className="mt-8 grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 grid md:grid-cols-2 gap-5">
              <QA
                q={<span className="inline-flex items-center gap-2"><QrCode className="h-4 w-4 opacity-90" />How do I buy a FastBit key?</span>}
                a={
                  <>
            ...
            {/* Side panel: Activation quick steps */}
            <aside className="rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <ol className="mt-3 space-y-2 text-sm list-decimal list-inside text-white/80">
                <li>
                  Log in to <a href="https://fastbit.cc/" target="_blank" rel="noopener noreferrer" className="underline decoration-white/20">fastbit.cc</a>
                </li>
                <li>Open <strong>My Account</strong></li>
...
function QA({ q, a }: { q: React.ReactNode; a: React.ReactNode }) {
  return (
    <details className="group rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-sm hover:border-white/20 transition overflow-hidden">
      ...
      <div className="h-px w-full bg-gradient-to-r from-[#05B565]/40 via-emerald-500/30 to-[#05B565]/40" />
      <div className="px-4 pb-4 pt-3 text-white/80 text-sm">{a}</div>
    </details>
  );
}
function BG() {
  // brand background swirls: deep slate + subtle green light
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      <div className="absolute -top-1/3 left-1/2 -translate-x-1/2 h-[80vh] w-[120vw] rounded-full bg-gradient-to-br from-[#39505A]/25 via-[#223139]/20 to-[#39505A]/25 blur-3xl" />
      <div className="absolute bottom-[-30vh] right-[-10vw] h-[60vh] w-[60vw] rounded-full bg-gradient-to-br from-[#05B565]/10 via-[#39505A]/10 to-[#05B565]/15 blur-3xl" />
    </div>
  );
}
