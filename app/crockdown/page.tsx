...
/** CrockDown — premium keys checkout page (blue vibe) */

/* ── Pricing source (same as your other pages) ─────────────────────────── */
const COINGECKO_IDS = {
  BTC:  'bitcoin',
  ETH:  'ethereum',
  SOL:  'solana',
...
/* ── CrockDown plans ───────────────────────────────────────────────────── */
const PLANS = [
  { id: 'cd-30',  label: '30 Days',  days: 30,  priceUSD:  5.99, wasUSD:  9.99,  note: '100 GB/day' },
  { id: 'cd-90',  label: '90 Days',  days: 90,  priceUSD: 17.99, wasUSD: 24.99,  note: '100 GB/day' },
  { id: 'cd-180', label: '180 Days', days: 180, priceUSD: 29.99, wasUSD: 49.99,  note: '100 GB/day' },
  { id: 'cd-365', label: '365 Days', days: 365, priceUSD: 49.99, wasUSD: 74.99,  note: '100 GB/day' },
] as const;
...
export default function Page(){
  ...
  return (
    <div className="min-h-screen bg-[#0a111b] text-white">
      <BG />
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-black/30 border-b border-white/10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 group" aria-label="Only.Exchange — Home" prefetch={false}>
            <span className="inline-grid h-8 w-8 place-items-center rounded-xl bg-gradient-to-br from-sky-500 via-blue-600 to-indigo-700 text-white ring-1 ring-white/20 shadow-sm transition-transform group-hover:scale-105">
              <ArrowLeftRight className="h-4 w-4" />
            </span>
            <span className="font-semibold group-hover:text-white">Only.Exchange</span>
          </Link>
          <a href="#checkout" onClick={e=>{e.preventDefault(); scrollToId('checkout')}} className="hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-sky-500 to-indigo-600">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 md:py-24">
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{duration:0.6}} className="max-w-3xl">
            ...
            <h1 className="text-4xl md:text-6xl font-bold leading-tight">
              CrockDown <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-300 via-blue-300 to-indigo-300">Premium Keys</span>
            </h1>
            <p className="mt-4 text-white/80 text-lg max-w-2xl">
              Pay with crypto and get your CrockDown premium key <em>instantly</em> after confirmation.
            </p>

            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#plans" onClick={e=>{e.preventDefault(); scrollToId('plans')}} className="px-5 py-3 rounded-2xl bg-gradient-to-r from-sky-500 to-indigo-600 inline-flex items-center gap-2 hover:from-sky-400 hover:to-indigo-500">
            ...
            <div className="mt-4 text-xs text-white/60">30:00 price lock • Sender pays network fees • No KYC</div>
          </motion.div>
        </div>
      </section>
      ...
      <section id="plans" className="py-14 border-t border-white/10 bg-white/5">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-white/70 mt-2">100 GB/day on all plans • Instant delivery after confirmations.</p>
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {PLANS.map((p) => (
              <motion.button
                ...
                className={`text-left rounded-2xl border ${selected.id===p.id? 'border-sky-300/70' : 'border-white/10'} bg-gradient-to-br from-white/10 to-transparent p-5`}
              >
      ...
      <section id="features" className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Feature icon={<HardDrive className="h-5 w-5" />} title="Unlimited storage" text="No enforced storage cap. Keep your files online—forever." />
            <Feature icon={<FolderDown className="h-5 w-5" />} title="Unlimited file size" text="Max upload size: unlimited; upload large files without worry." />
            <Feature icon={<Gauge className="h-5 w-5" />} title="100 GB/day traffic" text="All plans include 100 GB daily download allowance." />
            <Feature icon={<Download className="h-5 w-5" />} title="Accelerators & resume" text="Supports download accelerators and resumable downloads." />
            <Feature icon={<Ban className="h-5 w-5" />} title="No delays, no captcha" text="Skip queues, timers, and captchas entirely." />
            <Feature icon={<BadgeCheck className="h-5 w-5" />} title="No ads" text="A clean, distraction-free experience across the site." />
          </div>
        </div>
      </section>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-white/70">Buy CrockDown Premium Key direct to Email.</p>
              </div>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`px-4 py-2 rounded-2xl border text-sm inline-flex items-center gap-2
                      ${active ? "border-sky-300/70 bg-white/10" : "border-white/10 hover:border-white/30 bg-white/5"}`}
                  >
                    ...
                    <button
                      ...
                      className={`w-full px-6 py-4 rounded-2xl inline-flex items-center justify-center gap-2 text-lg
                        ${(!isEmailValid || (methodNeedsLivePrice(method) && !pricesUSD[method]) || (METHOD_NEEDS_CHAIN[method] && !chain) || generating)
                          ...
                          : "bg-gradient-to-r from-sky-600 via-blue-600 to-indigo-700 hover:from-sky-500 hover:to-indigo-600 shadow-[0_0_25px_rgba(56,189,248,0.35)]"}`}
                    >
                ...
                <div className="space-y-4">
                  ...
                  <ol className="space-y-3 text-white/80 text-sm">
                    <li className="flex items-center gap-3">
                      ...
                      <span>Select a CrockDown Premium plan</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center">
                    ...
                    <li className="flex items-center gap-3">
                      ...
                      <span>Choose coin (and network for USDT/USDC or ETH L2)</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center">
      ...
      <section id="faq" className="py-16 border-t border-white/10 bg-gradient-to-b from-transparent via-white/[0.05] to-transparent">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-bold">
            FAQ
            <span className="ml-3 inline-block align-middle h-2 w-24 rounded-full bg-gradient-to-r from-sky-500/70 via-blue-500/60 to-indigo-500/70" />
          </h2>

          <div className="mt-8 grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 grid md:grid-cols-2 gap-5">
              <QA
                q={<span className="inline-flex items-center gap-2"><QrCode className="h-4 w-4 opacity-90"/>How do I buy a CrockDown key?</span>}
                a={
                  <>
                    <ol className="list-decimal list-inside space-y-1">
                      <li>Choose a pack & enter your email above.</li>
                      <li>Select coin (and network for USDT/USDC or ETH L2).</li>
                      <li>Click <em>Generate</em> to lock your price and get an address.</li>
              ...
              <QA
                ...
                a="Instantly after required confirmations (usually minutes). We send the key to the email you entered at checkout."
              />

              <QA
                q={<span className="inline-flex items-center gap-2"><ShieldCheck className="h-4 w-4 opacity-90" />How to activate my CrockDown key?</span>}
                a={
                  <>
                    <div className="rounded-xl border border-white/10 bg-gradient-to-br from-sky-600/10 via-blue-600/10 to-indigo-600/10 p-3">
                    ...
                    <ol className="mt-3 list-decimal list-inside space-y-1">
                      <li>Log in to your CrockDown account (<a href="https://crockdown.com" target="_blank" rel="noreferrer" className="underline decoration-white/20">crockdown.com</a>).</li>
                      <li>Open the <a href="https://crockdown.com/account/" target="_blank" rel="noreferrer" className="underline decoration-white/20">My Account</a> page.</li>
                      <li>Find the <em>Apply Premium Key</em> field, paste your key, then click <em>Apply</em>.</li>
                    </ol>
                  </>
                }
              />
              ...
              <QA
                ...
                a={
                  <>
                    ...
                    <div className="mt-1">ETH also supports L2s (Base, Arbitrum, Optimism, Polygon, zkSync, Linea, Scroll).</div>
                  </>
                }
              />
              ...
              <QA
                ...
                a={
                  <ul className="list-disc list-inside space-y-1">
                    ...
                    <li><strong>Wrong network:</strong> For USDT/USDC, network must match. If unsure, reach out ASAP.</li>
                  </ul>
                }
              />
              ...
              <QA
                ...
                a="During payment, email is temporarily locked. Click “Cancel / Start Over” to change details and generate again."
              />
            </div>
            ...
            <aside className="rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <ul className="mt-3 space-y-2 text-sm">
                <li>✔️ <a href="/crockdown?plan=cd-90#checkout" className="underline decoration-white/20 hover:decoration-white">90-day pack (deep link)</a></li>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 text-sm text-white/70">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center gap-2">
              <span className="inline-flex h-6 w-6 rounded-lg bg-gradient-to-br from-sky-500 via-blue-600 to-indigo-700" />
...
function QA({ q, a }: { q: React.ReactNode; a: React.ReactNode }) {
  return (
    <details className="group rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-sm hover:border-white/20 transition overflow-hidden">
      ...
      <div className="h-px w-full bg-gradient-to-r from-sky-500/40 via-blue-500/30 to-indigo-500/40" />
...
function BG(){
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      <div className="absolute -top-1/3 left-1/2 -translate-x-1/2 h-[80vh] w-[120vw] rounded-full bg-gradient-to-br from-sky-600/25 via-blue-700/15 to-indigo-700/25 blur-3xl"/>
      <div className="absolute bottom-[-30vh] right-[-10vw] h-[60vh] w-[60vw] rounded-full bg-gradient-to-br from-indigo-700/15 via-sky-600/10 to-blue-700/15 blur-3xl"/>
    </div>
  );
}
