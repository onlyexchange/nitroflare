...
/** ───────────────────────────────────────────────────────────────────────────
 *  SubyShare — blue hero + white/glass content
 *  Palette:
 *    Primary:        #1AA3CD
 *    Primary (dark): #1489AD
 *    Ink:            #0A2B3B
 *    Card edge:      rgba(26,163,205,0.18)
...
// Packs
const PLANS = [
  { id: 'ss-30',  label: '30 Days',  days: 30,  priceUSD: 12.95, wasUSD: 19.99 },
  { id: 'ss-75',  label: '75 Days',  days: 75,  priceUSD: 22.95, wasUSD: 29.95 },
  { id: 'ss-180', label: '180 Days', days: 180, priceUSD: 34.95, wasUSD: 47.45 },
  { id: 'ss-365', label: '365 Days', days: 365, priceUSD: 72.95, wasUSD: 87.95 },
] as const;
...
export default function Page(){
  ...
  return (
    <div className="min-h-screen" style={{ color: COLORS.ink, background: '#F6FAFD' }}>
      {/* Header */}
      <header className="sticky top-0 z-40 backdrop-blur bg-white/70 border-b" style={{ borderColor: COLORS.cardEdge }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 group" prefetch={false} aria-label="Only.Exchange — Home">
      ...
      <section className="relative"
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14 md:py-20">
          <motion.div initial={{opacity:0, y:18}} animate={{opacity:1, y:0}} transition={{duration:0.5}} className="max-w-3xl text-white">
            <div className="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs bg-white/15 ring-1 ring-white/20 mb-6">
              <Zap className="h-3.5 w-3.5"/> Instant delivery • Live price lock • Unique address
            </div>
            <h1 className="text-4xl md:text-6xl font-bold leading-tight">
              SubyShare <span className="opacity-90">Premium Keys</span>
            </h1>
            <p className="mt-4 text-white/90 text-lg max-w-2xl">
              Buy with crypto and get your key by email as soon as confirmations land.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#plans" onClick={(e)=>{e.preventDefault(); document.getElementById('plans')?.scrollIntoView({behavior:'smooth'})}}
                 className="px-5 py-3 rounded-2xl inline-flex items-center gap-2 bg-white/15 ring-1 ring-white/25 hover:bg-white/20">
                View Plans <ArrowRight className="h-4 w-4"/>
              </a>
              <a href="#checkout" onClick={(e)=>{e.preventDefault(); document.getElementById('checkout')?.scrollIntoView({behavior:'smooth'})}}
                 className="px-5 py-3 rounded-2xl inline-flex items-center gap-2 bg-white text-[color:var(--ink)]"
                 style={{ ['--ink' as any]: COLORS.ink }}>
                <Bitcoin className="h-4 w-4"/> Pay with Crypto
              </a>
            </div>
          </motion.div>
      ...
      <section id="plans" className="py-14 border-t" style={{ borderColor: COLORS.cardEdge }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-[color:var(--ink)]/70 mt-2">40GB/day • No ads or waiting • Lifetime storage • 10MB/s</p>
          ...
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {PLANS.map((p) => (
              <motion.button
                ...
                className="text-left rounded-2xl border backdrop-blur p-5"
                style={{
                  background: COLORS.glass,
                  borderColor: COLORS.cardEdge,
                  boxShadow: selected.id===p.id
                    ? `0 0 0 2px ${COLORS.primary} inset, 0 8px 24px rgba(26,163,205,0.15)`
                    : '0 8px 24px rgba(26,163,205,0.08)',
                }}
              >
                <div className="flex items-center justify-between">
                  ...
                  {selected.id===p.id && <CheckCircle2 className="h-5 w-5" style={{ color: '#69C16F' }}/>}
                </div>
                <div className="mt-1 text-lg font-semibold" style={{ color: COLORS.ink }}>
                  {p.label}
                </div>
                <div className="mt-2 flex items-baseline gap-2">
                  <div className="text-3xl font-bold" style={{ color: COLORS.ink }}>${p.priceUSD.toFixed(2)}</div>
      ...
      <section id="features" className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-bold" style={{ color: COLORS.primaryDark }}>
            💎 Why Go Premium with SubyShare.com?
          </h2>
          <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Feature icon={<Gauge className="h-5 w-5" />} title="10MB/s download speed" text="Consistently fast downloads for large files." />
            <Feature icon={<Zap className="h-5 w-5" />} title="No waiting, no captchas" text="Skip queues and start instantly." />
            <Feature icon={<ShieldCheck className="h-5 w-5" />} title="Direct downloads" text="Start downloads straight from your account." />
            <Feature icon={<Coins className="h-5 w-5" />} title="40GB/day" text="Plenty of bandwidth for your daily needs." />
            <Feature icon={<HardDrive className="h-5 w-5" />} title="Lifetime storage" text="Keep your files available long-term." />
            <Feature icon={<Mail className="h-5 w-5" />} title="Instant key delivery" text="We email your key after confirmations." />
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                <h3 className="text-4xl md:text-5xl font-extrabold tracking-tight" style={{ color: COLORS.ink }}>Checkout</h3>
                <p className="mt-2 text-[color:var(--ink)]/70">Buy SubyShare.com Premium key — delivered by email.</p>
              </div>
            </div>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      background: active ? `linear-gradient(180deg, ${COLORS.primary} 0%, ${COLORS.primaryDark} 100%)` : COLORS.glass,
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      <div className="flex-1 px-4 py-3 rounded-xl border text-lg" style={{ background: COLORS.glass, borderColor: COLORS.cardEdge }}>
                      ...
                      <button
                        ...
                        style={{ borderColor: COLORS.cardEdge, background: COLORS.glass }}
                      >
                        Change
                      </button>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      <input
                        ...
                        className="flex-1 px-4 py-3 rounded-xl border outline-none text-lg"
                        style={{
                          background: COLORS.glass,
                          borderColor:
                            email.length === 0
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="mt-1 flex gap-2">
                        <input readOnly value={lockedAmount} className="w-full px-4 py-3 rounded-xl border font-mono text-lg"
                               style={{ background: COLORS.glass, borderColor: COLORS.cardEdge }}/>
                    ...
                    <div>
                      ...
                      <div className="mt-1 flex gap-2">
                        <input readOnly value={address} className="w-full px-4 py-3 rounded-xl border font-mono text-sm"
                               style={{ background: COLORS.glass, borderColor: COLORS.cardEdge }}/>
                  ...
                  <div className="w-full flex items-center justify-center">
                    {qrURL() ? (
                      <img
                        ...
                        style={{ borderColor: COLORS.cardEdge, boxShadow: '0 0 35px rgba(26,163,205,0.15)' }}
              ...
              {step !== 'pay' ? (
                <div className="space-y-4">
                  ...
                  <ol className="space-y-3 text-[color:var(--ink)]/80 text-sm">
                    <Step icon={<Bitcoin className="h-4 w-4" />} text="Choose a plan above" />
                    <Step icon={<Mail className="h-4 w-4" />} text="Enter your email for delivery" />
                    <Step icon={<ShieldCheck className="h-4 w-4" />} text="Pick a coin (and network for ETH / USDT / USDC)" />
                  ...
                  <div className="rounded-2xl border p-4 text-xs text-[color:var(--ink)]/70"
                       style={{ background: COLORS.glass, borderColor: COLORS.cardEdge }}>
                    Payment details and QR appear after you generate an address.
                  </div>
                </div>
              ) : (
                <div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 grid md:grid-cols-2 gap-5">
                ...
                a={
                  <>
                    <ol className="list-decimal list-inside space-y-1">
                      <li>Choose a SubyShare plan above.</li>
              ...
              />
              <QA
                q={<><Mail className="h-4 w-4 opacity-90" /> How to activate my SubyShare key?</>}
                a={
                  <>
                    ...
                    <ol className="mt-2 list-decimal list-inside space-y-1">
                      <li>Log in to your SubyShare account (<a className="underline" href="https://subyshare.com/account/register" target="_blank" rel="noreferrer">create one here</a> if needed).</li>
            ...
            {/* Side panel */}
            <aside className="rounded-2xl border p-5" style={{ background: COLORS.glass, borderColor: COLORS.cardEdge }}>
              ...
              <ul className="mt-3 space-y-2 text-sm text-[color:var(--ink)]/80">
                <li>✔️ 40GB/day bandwidth</li>
                <li>✔️ Lifetime storage</li>
                <li>✔️ No ads • No waiting</li>
              </ul>
              <div className="mt-5 rounded-xl border p-4" style={{ background: COLORS.glass, borderColor: COLORS.cardEdge }}>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 text-sm text-[color:var(--ink)]/70">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold" style={{ color: COLORS.ink }}>Only.Exchange</span>
            </div>
  ...
  return (
    <li className="flex items-center gap-3">
      <div className="h-7 w-7 rounded-xl grid place-items-center"
           style={{ background: '#fff', border: `1px solid ${COLORS.cardEdge}`, color: COLORS.primaryDark }}>
        {icon}
      </div>
      <span>{text}</span>
    </li>
...
function Badge({ children }:{ children: React.ReactNode }){
  return (
    <div className="rounded-xl px-3 py-2 flex items-center justify-center"
         style={{ border: `1px solid ${COLORS.cardEdge}`, background: COLORS.glass }}>
      <ShieldCheck className="h-4 w-4 mr-2" style={{ color: '#69C16F' }}/>
...
function Stat({ label, value, mono=false }:{label:string; value:string; mono?:boolean}){
  return (
    <div>
      ...
      <div className={`mt-1 px-4 py-3 rounded-xl border text-lg ${mono ? 'font-mono' : ''}`}
           style={{ background: COLORS.glass, borderColor: COLORS.cardEdge }}>
        {value}
      </div>
    </div>
...
function QA({ q, a }: { q: React.ReactNode; a: React.ReactNode }) {
  return (
    <details className="group rounded-2xl border hover:bg-white/80 transition overflow-hidden"
             style={{ borderColor: COLORS.cardEdge, background: COLORS.glass }}>
      <summary className="cursor-pointer list-none flex items-center justify-between p-4" style={{ color: COLORS.ink }}>
