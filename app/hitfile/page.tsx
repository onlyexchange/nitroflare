...
/** Hitfile — white glass vibe + blue accents (#00A5B5) */
...
// Packs (standard + “Plus”)
const PLANS = [
  { id: 'hf-25',       label: '25 Days',   days: 25,  priceUSD: 5.95,  wasUSD: 9.95,  bandwidth: '300 GB' },
  { id: 'hf-70',       label: '70 Days',   days: 70,  priceUSD: 19.95, wasUSD: 24.95, bandwidth: '1400 GB' },
  { id: 'hf-150',      label: '150 Days',  days: 150, priceUSD: 39.95, wasUSD: 49.95, bandwidth: '3000 GB' },
  { id: 'hf-350',      label: '350 Days',  days: 350, priceUSD: 69.95, wasUSD: 89.95, bandwidth: '7000 GB' },
  { id: 'hf-25-plus',  label: '25 Days +', days: 25,  priceUSD: 7.95,  wasUSD: 12.45, bandwidth: '600 GB' },
  { id: 'hf-70-plus',  label: '70 Days +', days: 70,  priceUSD: 24.95, wasUSD: 30.45, bandwidth: '2800 GB' },
  { id: 'hf-150-plus', label: '150 Days +',days: 150, priceUSD: 49.95, wasUSD: 62.45, bandwidth: '6000 GB' },
  { id: 'hf-350-plus', label: '350 Days +',days: 350, priceUSD: 99.95, wasUSD: 112.45, bandwidth: '14000 GB' },
] as const;
...
export default function Page() {
  // Selection
  const [selected, setSelected]       = useState<Plan>(PLANS[1]); // default 70 Days (popular)
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#f7f8fb] via-[#f3f5f9] to-[#eef2f7] text-slate-900">
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-white/50 border-b border-black/5">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <a
            ...
            className="hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-xl text-white hover:opacity-95"
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14 md:py-20">
          <motion.div initial={{opacity:0, y:18}} animate={{opacity:1, y:0}} transition={{duration:0.5}} className="max-w-3xl">
            ...
            <h1 className="text-4xl md:text-6xl font-extrabold leading-tight tracking-tight">
              Hitfile <span style={{ color: ACCENT }}>Premium Keys</span>
            </h1>
            <p className="mt-4 text-slate-700 text-lg max-w-2xl">
              Pay with crypto and get your Hitfile premium key <em>instantly</em> after confirmations.
            </p>

            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#plans" onClick={e=>{e.preventDefault(); scrollToId('plans')}} className="px-5 py-3 rounded-2xl text-white inline-flex items-center gap-2 hover:opacity-95" style={{ backgroundColor: ACCENT }}>
              ...
              <a href="#checkout" onClick={e=>{e.preventDefault(); scrollToId('checkout')}} className="px-5 py-3 rounded-2xl border border-black/10 inline-flex items-center gap-2 bg-white/60 backdrop-blur hover:bg-white/80">
                <Bitcoin className="h-5 w-5" style={{ color: ACCENT }}/> <span className="font-semibold">Pay with Crypto</span>
              </a>
            </div>
      ...
      <section id="plans" className="py-14 border-t border-black/5">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-slate-600 mt-2">Upgrade to Premium and download without limits — max speed, ad-free, no timeouts.</p>
          ...
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {PLANS.map((p) => (
              <motion.button
                ...
                className={`text-left rounded-2xl border ${selected.id===p.id? 'border-[color:rgb(0,165,181)]/50' : 'border-black/10'} bg-white/60 backdrop-blur p-5`}
              >
                <div className="flex items-center justify-between">
                  <div className="text-xl font-semibold">{p.label}</div>
                  {selected.id===p.id && <CheckCircle2 className="h-5 w-5" style={{ color: ACCENT }}/>}
                </div>

                <div className="mt-2 flex items-baseline gap-2">
                  {p.wasUSD !== undefined && (
                    <div className="text-sm text-slate-500 line-through">${p.wasUSD.toFixed(2)}</div>
                  )}
      ...
      <section id="features" className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Feature icon={<Gauge className="h-5 w-5" style={{ color: ACCENT }} />} title="Maximum download speed" text="No throttling — get files as fast as your line allows." />
            <Feature icon={<ShieldCheck className="h-5 w-5" style={{ color: ACCENT }} />} title="Ad-free & no timeouts" text="Clean, uninterrupted downloads with no delays." />
            <Feature icon={<RefreshCw className="h-5 w-5" style={{ color: ACCENT }} />} title="Resume support" text="Resume on reconnect; great for large files." />
            <Feature icon={<Zap className="h-5 w-5" style={{ color: ACCENT }} />} title="Parallel downloads" text="Use multiple connections and download managers." />
            <Feature icon={<PlayCircle className="h-5 w-5" style={{ color: ACCENT }} />} title="Online video" text="Watch videos online (new)." />
            <Feature icon={<Mail className="h-5 w-5" style={{ color: ACCENT }} />} title="Instant delivery" text="Key emailed automatically after confirmations." />
          </div>
        </div>
      </section>
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                <h3 className="text-4xl md:text-5xl font-extrabold tracking-tight">
                  Checkout <span style={{ color: ACCENT }}>Hitfile</span>
                </h3>
                <p className="mt-2 text-slate-600">Buy Hitfile Premium Key direct to Email.</p>
              </div>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`px-4 py-2 rounded-2xl border text-sm inline-flex items-center gap-2
                      ${active
                        ? "border-[color:rgb(0,165,181)]/60 bg-white/70"
                        : "border-black/10 hover:border-black/20 bg-white/50"}`}
                  >
                    <Icon className="h-4 w-4" style={{ color: ACCENT }}/>{m.label}
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl border text-sm
                        ${active ? 'border-[color:rgb(0,165,181)]/60 bg-white/70'
                                 : 'border-black/10 hover:border-black/20 bg-white/50'}`}
                    >
                      {chainLabel(c)}
                    </button>
              ...
              {step !== 'pay' ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid sm:grid-cols-1 gap-3">
                    <button
                      ...
                      style={{ backgroundColor: (!isEmailValid || (methodNeedsLivePrice(method) && !pricesUSD[method]) || (METHOD_NEEDS_CHAIN[method] && !chain) || generating) ? undefined : ACCENT }}
                    >
                ...
                <div className="space-y-6">
                  <h4 className="text-2xl font-semibold flex items-center gap-2">
                    <QrCode className="h-5 w-5" style={{ color: ACCENT }}/> Payment Details
                  </h4>
                  ...
                  <div className="w-full flex items-center justify-center">
                    {qrURL() ? (
                      <img
                        ...
                        width={260}
                        height={260}
                        className="mt-2 rounded-xl border border-black/10 shadow-[0_0_35px_rgba(0,165,181,0.25)] bg-white/60 backdrop-blur"
                  ...
                  <ol className="space-y-3 text-slate-700 text-sm">
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/70 border border-black/10 grid place-items-center">
                        <Bitcoin className="h-4 w-4" style={{ color: ACCENT }} />
                      </div>
                      <span>Select a Hitfile Premium plan</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/70 border border-black/10 grid place-items-center">
                    ...
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/70 border border-black/10 grid place-items-center">
                        <TimerIcon className="h-4 w-4" style={{ color: ACCENT }} />
                      </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <h2 className="text-3xl md:text-4xl font-bold">
              FAQ
              <span className="ml-3 inline-block align-middle h-2 w-24 rounded-full" style={{backgroundColor: ACCENT}} />
            </h2>
          ...
          <div className="mt-8 grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 grid md:grid-cols-2 gap-5">
              <QA
                q={<span className="inline-flex items-center gap-2"><QrCode className="h-4 w-4" style={{ color: ACCENT }} />How do I buy a Hitfile key?</span>}
                a={
                  <>
              ...
              <QA
                q={<span className="inline-flex items-center gap-2"><ShieldCheck className="h-4 w-4" style={{ color: ACCENT }} />Which coins & networks do you support?</span>}
                a={
                  <>
              ...
              <QA
                q={<span className="inline-flex items-center gap-2"><Mail className="h-4 w-4" style={{ color: ACCENT }} />When will I receive my key?</span>}
                a={<>Instantly after required confirmations (usually minutes). We email the key to the address you enter at checkout.</>}
              />

              {/* Activation moved into FAQ */}
              <QA
                q={<span className="inline-flex items-center gap-2"><ShieldCheck className="h-4 w-4" style={{ color: ACCENT }} />How do I activate my Hitfile key?</span>}
                a={
                  <>
                    ...
                    <ol className="mt-3 list-decimal list-inside space-y-1">
                      <li>Log in to your Hitfile account (or register at <a href="https://hitfile.net/reg" target="_blank" rel="noreferrer" className="underline">hitfile.net/reg</a>).</li>
                      <li>Open <em>My Account</em>.</li>
                      <li>Find the <strong>Apply Promo Code</strong> field, paste your key, and apply.</li>
                    </ol>
                  </>
                }
              />
              ...
              <QA
                q={<span className="inline-flex items-center gap-2"><AlertTriangle className="h-4 w-4" style={{ color: ACCENT }} />I sent the wrong amount / network</span>}
                a={
              ...
              <QA
                q={<span className="inline-flex items-center gap-2"><Coins className="h-4 w-4" style={{ color: ACCENT }} />Where do your prices come from?</span>}
            ...
            <aside className="rounded-2xl border border-black/10 bg-white/60 p-5 backdrop-blur">
              ...
              <ul className="mt-3 space-y-2 text-sm text-slate-700">
                <li>✔️ Max speed, ad-free, no timeouts</li>
                <li>✔️ Download managers & resume support</li>
                <li>✔️ Parallel downloads</li>
