...
type Plan = {
  id: string;
  tier: 'Premium' | 'Premium Max';
  label: string;        // e.g. "30 Days"
  days: number;
  priceUSD: number;
  wasUSD?: number;
  bandwidth?: string;   // e.g. "100 GB/day"
  extras?: string[];    // small bullets like "Watch videos online"
};

const PLANS: Plan[] = [
  // Premium
  { id: 'fb-prem-30',  tier: 'Premium',     label: '30 Days',  days: 30,  priceUSD: 28.95, bandwidth: '20 GB/day' },
  { id: 'fb-prem-90',  tier: 'Premium',     label: '90 Days',  days: 90,  priceUSD: 71.95, bandwidth: '20 GB/day' },
  { id: 'fb-prem-365', tier: 'Premium',     label: '365 Days', days: 365, priceUSD: 189.95, bandwidth: '20 GB/day' },

  // Premium Max
  { id: 'fb-max-30',   tier: 'Premium Max', label: '30 Days',  days: 30,  priceUSD: 34.95, wasUSD: 46.95,  bandwidth: '100 GB/day', extras: ['Watch videos online'] },
  { id: 'fb-max-90',   tier: 'Premium Max', label: '90 Days',  days: 90,  priceUSD: 79.95, wasUSD: 109.95, bandwidth: '100 GB/day', extras: ['Watch videos online'] },
  { id: 'fb-max-365',  tier: 'Premium Max', label: '365 Days', days: 365, priceUSD: 199.95, wasUSD: 279.95, bandwidth: '100 GB/day', extras: ['Watch videos online'] },
];

/* ============== PAGE ============== */
export default function FboomPage() {
  const [selected, setSelected] = useState<Plan>(PLANS.find(p => p.id === 'fb-max-365')!); // default to Max 365
  ...
  useEffect(() => {
    ...
    if (!q) return;
    const norm = decodeURIComponent(q).toLowerCase();
    const target = PLANS.find(
      (p) => p.id.toLowerCase() === norm || p.label.toLowerCase() === norm
    );
    if (target) {
      resetPayment();
      setSelected(target);
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 md:py-24">
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{duration:0.6}} className="max-w-3xl">
            ...
            <p className="mt-4 text-white/80 text-lg max-w-2xl">
              ...
              Premium Max unlocks up to <strong>100&nbsp;GB/day</strong> and video streaming.
            </p>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-7">
            <div className="flex items-center gap-2">
              <div className="h-2 w-2 rounded-full" style={{ background: brandOrange }} />
              <h3 className="text-xl font-semibold">Premium (20 GB/day)</h3>
            </div>
            <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {PLANS.filter(p => p.tier === 'Premium').map((p) => {
                const active = selected.id === p.id;
                const best = p.id === 'fb-prem-365';
                return (
                  <motion.button
                    key={p.id}
                    onClick={() => handleSelectPlan(p)}
                    whileHover={{ scale: 1.02 }}
                    className={`relative text-left rounded-2xl border ${active ? 'border-[rgba(255,166,72,0.7)]' : 'border-white/10'} bg-gradient-to-br from-white/10 to-transparent p-5`}
                  >
                    {best && (
                      <span className="absolute -top-2 -right-2 inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-semibold bg-[#FFA648] text-black">
                        <Star className="h-3 w-3" /> Best Value
                      </span>
                    )}
                    <div className="flex items-center justify-between">
                      <div className="text-xl font-semibold">{p.label}</div>
                      {active && <CheckCircle2 className="h-5 w-5" style={{ color: brandOrange }} />}
                    </div>
          ...
          <div className="mt-10">
            <div className="flex items-center gap-2">
              <div className="h-2 w-2 rounded-full" style={{ background: brandOrange }} />
              <h3 className="text-xl font-semibold">Premium Max (100 GB/day & Video)</h3>
            </div>
            <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {PLANS.filter(p => p.tier === 'Premium Max').map((p) => {
                const active = selected.id === p.id;
                const best = p.id === 'fb-max-365';
                return (
                  <motion.button
                    key={p.id}
                    onClick={() => handleSelectPlan(p)}
                  ...
                  >
                    ...
                    <div className="mt-3 text-xs text-white/70 space-y-1">
                      {p.bandwidth && <div>{p.bandwidth}</div>}
                      {p.extras?.map((x) => <div key={x} className="inline-flex items-center gap-1"><Film className="h-3.5 w-3.5 opacity-70" /> {x}</div>)}
                    </div>
                  </motion.button>
                );
              })}
          ...
          <div className="mt-6 text-sm text-white/70">
            Selected: <span className="text-white font-medium">{selected.tier} — {selected.label}</span>
          </div>
        </div>
      </section>
      ...
      <section id="features" className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Feature icon={<ListChecks className="h-5 w-5" />} title="Resumable downloads" text="Stop and resume without losing progress." />
            <Feature icon={<ShieldCheck className="h-5 w-5" />} title="Virus-checked files" text="Safer downloads across the platform." />
            <Feature icon={<Ban className="h-5 w-5" />} title="Ad-free" text="No ads, no distraction, just files." />
            <Feature icon={<Zap className="h-5 w-5" />} title="Fast from 200+ partner sites" text="Stable speeds across supported mirrors." />
          </div>

          <h3 className="text-2xl font-semibold mt-10">💎 Premium Max Adds</h3>
          <div className="mt-4 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Feature icon={<Film className="h-5 w-5" />} title="Watch videos online" text="Stream directly without downloading first." />
            <Feature icon={<Rocket className="h-5 w-5" />} title="Exclusive high-speed servers" text="Priority routes for maximum speeds." />
            <Feature icon={<Download className="h-5 w-5" />} title="Accelerators & simultaneous DLs" text="Multi-connection support at full tilt." />
          </div>
        </div>
      </section>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-white/70">Buy Fboom Premium Key — instant email delivery.</p>
              </div>
            </div>
            ...
            <div className="mt-10 grid lg:grid-cols-2 gap-8">
              {/* LEFT: before payment or during pay */}
              {step !== 'pay' ? (
                    ...
                    <button
                      ...
                      className={`w-full px-6 py-4 rounded-2xl inline-flex items-center justify-center gap-2 text-lg ${
                        (!isEmailValid || (methodNeedsLivePrice(method) && !pricesUSD[method]) || (METHOD_NEEDS_CHAIN[method] && !chain) || generating)
                          ...
                          : `shadow-[0_0_25px_rgba(255,166,72,0.35)] ${brandChip}`
                      }`}
                    >
                  ...
                  <div className="w-full flex items-center justify-center">
                    {qrURL() ? (
                      <img
                        ...
                        width={260}
                        height={260}
                        className="mt-2 rounded-xl border border-white/10 shadow-[0_0_35px_rgba(255,166,72,0.25)]"
              ...
              {step !== 'pay' ? (
                <div className="space-y-4">
                  ...
                  <ol className="space-y-3 text-white/80 text-sm">
                    <li className="flex items-center gap-3">
                      ...
                      <span>Select a Fboom Premium plan</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center">
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm">
                    <div className="font-semibold">Activate your Fboom Key</div>
                    ...
                    <ol className="mt-2 list-decimal list-inside text-white/70 space-y-1">
                      <li>Log in at <a href="https://fboom.me/auth/register" target="_blank" rel="noopener noreferrer" className="underline decoration-white/20">fboom.me</a></li>
                      <li>Open <em>My Account</em></li>
                      <li>Paste your Premium Key into the <strong>Premium Key</strong> field and click <strong>Apply</strong></li>
                    </ol>
                  </div>
                </div>
              ) : (
                <div>
                  ...
                  <div className="mt-4 grid sm:grid-cols-2 gap-4">
                    <Summary label="Pack" value={`${selected.tier} — ${selected.label}`} />
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 grid md:grid-cols-2 gap-5">
              <QA
                q={<span className="inline-flex items-center gap-2"><QrCode className="h-4 w-4 opacity-90" />How do I buy a Fboom key?</span>}
                a={
                  <>
...
function BG() {
  // dark greys with orange highlight bloom
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      <div className="absolute -top-1/3 left-1/2 -translate-x-1/2 h-[80vh] w-[120vw] rounded-full bg-gradient-to-br from-zinc-700/25 via-zinc-900/20 to-[#FFA648]/20 blur-3xl" />
      <div className="absolute bottom-[-30vh] right-[-10vw] h-[60vh] w-[60vw] rounded-full bg-gradient-to-br from-[#FFA648]/18 via-zinc-700/15 to-zinc-900/15 blur-3xl" />
    </div>
  );
}
