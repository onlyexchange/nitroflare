...
import { Bitcoin, Zap, ShieldCheck, Rocket, CheckCircle2, Copy, QrCode, Timer as TimerIcon, Flame, ArrowRight } from "lucide-react";

// Use your server routes we set up:
const COINGECKO_URL = '/api/price?ids=bitcoin';
const WALLETS_URL = '/api/next-btc-address';

import { useEffect, useMemo, useState } from "react";
import { motion } from "framer-motion";
import { Bitcoin, Zap, ShieldCheck, Rocket, CheckCircle2, Copy, QrCode, Mail, TimerIcon, Flame, ArrowRight } from "lucide-react";
 ...
 *
 * Live BTC pricing via your proxy endpoint (defaults to /api/proxy-price.php?ids=bitcoin)
 * Unique BTC receiving address via /api/generate-wallet.php?coin=BTC (JSON {address:"..."})
 *
 * How to use:
 * - Drop into Next.js page (app/page.tsx or pages/index.tsx) or any React app with Tailwind.
 * - Ensure PRICE_PROXY and WALLET_ENDPOINT env/vars or use defaults below.
 * - Replace copy, logos, and legal text to match your brand (Only.Exchange / BuyPremiumKeys, etc.).
 */

const COINGECKO_URL = 'https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd';
const WALLETS_URL = '/api/bitcoin-wallets.json';
...
export default function NitroflareDegenLanding(){
  ...
  const [timer, setTimer] = useState(29 * 60 + 59); // 29:59 FOMO timer for deal
  ...
  // Compute amount in BTC (truncate to 8 decimals, no rounding)
  ...
  async function startPayment(){
    ...
    try{
      ...
      const data = await res.json();
const arr = Array.isArray(data?.wallets) ? data.wallets : [];
...
if (!addr) throw new Error('No wallet available');
      ...
      setAmountBTC(computedAmount);
      setStep('pay');
      setStatus('Demo address loaded. Replace WALLET_ENDPOINT for production.');
    }
  }
    ...
    <div className="min-h-screen bg-[#0b0b12] text-white">
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-black/30 border-b border-white/10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <a href="#checkout" className="hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-purple-500 to-indigo-500"><Bitcoin className="h-4 w-4"/> Pay with Bitcoin</a>
        </div>
      </header>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 md:py-24">
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{duration:0.6}} className="max-w-3xl">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#plans" className="px-5 py-3 rounded-2xl bg-gradient-to-r from-purple-500 to-indigo-500 inline-flex items-center gap-2 hover:from-purple-400 hover:to-indigo-400"><Flame className="h-5 w-5"/> View Plans</a>
              <a href="#checkout" className="px-5 py-3 rounded-2xl border border-white/15 hover:border-white/30 inline-flex items-center gap-2"><Bitcoin className="h-5 w-5"/> Pay with BTC</a>
            </div>
          </motion.div>
        </div>
      </section>
            ...
            <div className="rounded-3xl border border-white/10 bg-black/40 p-6">
              ...
              <div className="mt-5 grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="mt-1 px-3 py-2 rounded-xl bg-white/5 border border-white/10">{selected.label} — ${selected.priceUSD.toFixed(2)}</div>
                </div>
                <div>
                  <label className="text-sm text-white/70">Your Email (for key delivery)</label>
                  <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@email.com" className="mt-1 w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 outline-none"/>
                </div>
              </div>
              ...
              <div className="mt-5 grid sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <div className="mt-1 px-3 py-2 rounded-xl bg-white/5 border border-white/10 font-mono">{btcUSD? `$${btcUSD.toLocaleString()}` : '—'}</div>
                </div>
                <div>
                  <div className="text-sm text-white/70">Amount in BTC</div>
                  <div className="mt-1 px-3 py-2 rounded-xl bg-white/5 border border-white/10 font-mono">{computedAmount || '—'}</div>
                </div>
                <div>
                  <div className="text-sm text-white/70">Savings today</div>
                  <div className="mt-1 px-3 py-2 rounded-xl bg-white/5 border border-white/10">Save ${(selected.wasUSD - selected.priceUSD).toFixed(2)}</div>
                </div>
              </div>

              <button onClick={startPayment} className="mt-5 w-full px-5 py-3 rounded-2xl bg-gradient-to-r from-purple-500 to-indigo-500 inline-flex items-center justify-center gap-2 hover:from-purple-400 hover:to-indigo-400">
                <Rocket className="h-5 w-5"/> Generate address & start payment
              </button>
              <p className="mt-3 text-xs text-white/60">By continuing you agree to our Terms. Prices shown in USD; you will pay the BTC equivalent at current rate.</p>
            </div>
                ...
                <div className="mt-4 grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="mt-1 flex gap-2">
                      ...
                      <button onClick={()=>copy(amountBTC)} className="px-3 rounded-xl border border-white/10 hover:border-white/20"><Copy className="h-4 w-4"/></button>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="mt-1 flex gap-2">
                      ...
                      <button onClick={()=>copy(address)} className="px-3 rounded-xl border border-white/10 hover:border-white/20"><Copy className="h-4 w-4"/></button>
                    </div>
                  </div>
                  <div className="md:col-span-2 flex items-center justify-center">
                    {address ? (
                      ...
                      <div className="mt-2 h-[260px] w-[260px] rounded-xl border border-dashed border-white/10 grid place-items-center text-white/40">QR will appear here</div>
                    )}
                  </div>
                </div>
              )}
      ...
      <footer className="border-t border-white/10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 text-sm text-white/70">
          ...
          <p className="mt-6 text-xs text-white/60 max-w-4xl">NitroFlare is a third‑party service. This site sells access codes/keys only. All brand names and logos are property of their respective owners. Ensure you comply with all local laws and terms of service when using file hosting services.</p>
        </div>
      </footer>
    </div>
  );
