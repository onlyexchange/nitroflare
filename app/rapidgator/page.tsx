...
// Rapidgator plans (ids use "rg-*")
const PLANS = [
  { id: 'rg-30',  label: '30 Days',  days: 30,  priceUSD: 10.49, wasUSD: 14.99, note: '1 TB Bandwidth / 1 TB Storage' },
  { id: 'rg-90',  label: '90 Days',  days: 90,  priceUSD: 27.99, wasUSD: 39.99, note: '4 TB Bandwidth / 3 TB Storage' },
  { id: 'rg-180', label: '180 Days', days: 180, priceUSD: 34.99, wasUSD: 49.99, note: '6 TB Bandwidth / 6 TB Storage' },
  { id: 'rg-365', label: '365 Days', days: 365, priceUSD: 69.99, wasUSD: 99.99, note: '12 TB Bandwidth / 12 TB Storage' },
] as const;
...
// Brand accent (orange)
const BRAND_RING   = 'from-orange-500/80 via-amber-500/70 to-orange-600/80';
const BRAND_FILL   = 'from-orange-500 to-amber-500';
const BRAND_SHADOW = 'shadow-[0_0_26px_rgba(255,134,27,0.4)]';

export default function RapidgatorPage() {
    ...
    <div className="min-h-screen bg-[#0b0b12] text-white">
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-black/30 border-b border-white/10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <a
            ...
            className={`hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r ${BRAND_FILL}`}
          >
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 md:py-24">
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{duration:0.6}} className="max-w-3xl">
            ...
            <p className="mt-4 text-white/80 text-lg max-w-2xl">
              Pay with crypto and get your Rapidgator premium key <em>instantly</em> after confirmations.
            </p>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                ...
                className={`px-5 py-3 rounded-2xl bg-gradient-to-r ${BRAND_FILL} inline-flex items-center gap-2 hover:brightness-110 ${BRAND_SHADOW}`}
              >
      ...
      <section id="plans" className="py-14 border-t border-white/10 bg-white/5">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-white/70 mt-2">Save more on longer plans. Keys deliver instantly after confirmations.</p>
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {PLANS.map((p) => (
              <motion.button
                ...
                className={`text-left rounded-2xl border ${selected.id===p.id? 'border-orange-400/70' : 'border-white/10'} bg-gradient-to-br from-white/10 to-transparent p-5`}
              >
                <div className="flex items-center justify-between">
                  <div className="text-xl font-semibold">{p.label}</div>
                  {selected.id===p.id && <CheckCircle2 className="h-5 w-5 text-orange-300"/>}
                </div>
      ...
      <section id="features" className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-bold">💎 Rapidgator Premium Benefits</h2>
          <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Feature icon={<Zap className="h-5 w-5" />} title="Full-speed downloads" text="Unlimited bandwidth with no throttling during peak hours." />
            <Feature icon={<Download className="h-5 w-5" />} title="Big file sizes" text="Download files up to 80 GB without restrictions." />
            <Feature icon={<ShieldCheck className="h-5 w-5" />} title="Private & secure" text="Ad-free experience and encrypted HTTPS access." />
            <Feature icon={<Gauge className="h-5 w-5" />} title="Skip the line" text="No queues, no waiting, no captchas—just click and go." />
            <Feature icon={<Mail className="h-5 w-5" />} title="Instant email delivery" text="Keys are dispatched automatically after confirmations." />
            <Feature icon={<Coins className="h-5 w-5" />} title="Crypto friendly" text="BTC, ETH (+ L2s), SOL, BNB, LTC, plus USDT/USDC." />
          </div>
        </div>
      </section>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-white/70">Buy Rapidgator Premium Key — email delivery.</p>
              </div>
            </div>
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl border text-sm
                        ${active ? 'border-orange-400/70 bg-white/10'
                                 : 'border-white/10 hover:border-white/30 bg-white/5'}`}
                    >
                      {chainLabel(c)}
                    </button>
              ...
              {step !== 'pay' ? (
                <div className="space-y-4">
                  <div>
                    ...
                    {selected.note && <div className="mt-1 text-xs text-white/60">{selected.note}</div>}
                  </div>
                    ...
                    <button
                      ...
                      className={`w-full px-6 py-4 rounded-2xl inline-flex items-center justify-center gap-2 text-lg
                        ${(!isEmailValid || (methodNeedsLivePrice(method) && !pricesUSD[method]) || (METHOD_NEEDS_CHAIN[method] && !chain) || generating)
                          ...
                          : `bg-gradient-to-r ${BRAND_FILL} hover:brightness-110 ${BRAND_SHADOW}`}`}
                    >
                ...
                <div className="space-y-4">
                  ...
                  <ol className="space-y-3 text-white/80 text-sm">
                    <li className="flex items-center gap-3">
                      ...
                      <span>Select a Rapidgator Premium plan</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="h-7 w-7 rounded-xl bg-white/5 border border-white/10 grid place-items-center">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <h2 className="text-3xl md:text-4xl font-bold">
              FAQ
              <span className="ml-3 inline-block align-middle h-2 w-24 rounded-full bg-gradient-to-r from-orange-500/70 via-amber-500/60 to-orange-400/70" />
            </h2>

            <div className="flex flex-wrap gap-2">
              <a
                href="/support"
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white bg-gradient-to-r ${BRAND_FILL} hover:brightness-110 ${BRAND_SHADOW}`}
              >
          ...
          <div className="mt-8 grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 grid md:grid-cols-2 gap-5">
              <QA
                ...
                a={
                  <>
                    ...
                    <p className="mt-2 text-white/70 text-xs">Questions? <a href="/support" className="underline decoration-white/20 hover:decoration-white">Contact support</a>.</p>
                  </>
                }
              />
              ...
              <QA
                q={<span className="inline-flex items-center gap-2"><ShieldCheck className="h-4 w-4 opacity-90"/> Wrong amount or network?</span>}
                a={
                  <>
                    <ul className="list-disc list-inside space-y-1">
                      <li><strong>Underpaid:</strong> send the difference to the same address before the timer expires.</li>
                      <li><strong>Overpaid:</strong> <a href="/support" className="underline decoration-white/20 hover:decoration-white">Open a ticket</a> with your TX hash—we’ll reconcile.</li>
                      <li><strong>Wrong network:</strong> For USDT/USDC, the network must match. If unsure, <a href="/support" className="underline decoration-white/20 hover:decoration-white">reach out</a> ASAP.</li>
                    </ul>
                  </>
                }
              />
              ...
              <QA
                ...
                a={
                  <>
                    <div className="rounded-xl border border-white/10 bg-gradient-to-br from-orange-500/10 via-amber-500/10 to-orange-400/10 p-3">
                    ...
                    <ol className="mt-3 list-decimal list-inside space-y-1">
                      <li>Log in to your Rapidgator account (<a href="https://rapidgator.net/" target="_blank" rel="noopener noreferrer" className="underline decoration-white/20 hover:decoration-white">register</a> if needed).</li>
                      <li>Open the <a href="https://rapidgator.net/coupon" target="_blank" rel="noopener noreferrer" className="underline decoration-white/20 hover:decoration-white">Coupons</a> page.</li>
              ...
              <QA
                ...
                a={
                  <>
                    ...
                    <div className="mt-1">Still missing? <a href="/support" className="underline decoration-white/20 hover:decoration-white">Open a ticket</a> with your order email and TX hash so we can look it up immediately.</div>
                  </>
                }
              />
            </div>
            ...
            <aside className="rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <ul className="mt-3 space-y-2 text-sm">
                <li>✔️ <a href="#plans" onClick={(e)=>{e.preventDefault(); scrollToId('plans');}} className="underline decoration-white/20 hover:decoration-white">Plans & pricing</a></li>
                <li>✔️ <a href="?plan=rg-90#checkout" className="underline decoration-white/20 hover:decoration-white">90-day pack (deep link)</a></li>
              ...
              <div className={`mt-5 rounded-xl border border-white/10 bg-gradient-to-br from-orange-500/10 via-amber-500/10 to-orange-400/10 p-4`}>
                ...
                <a
                  href="/support"
                  className={`mt-3 inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold text-white bg-gradient-to-r ${BRAND_FILL} hover:brightness-110 ${BRAND_SHADOW}`}
                >
                  <Mail className="h-4 w-4" />
                  Contact support
                </a>
...
function QA({ q, a }: { q: React.ReactNode; a: React.ReactNode }) {
  return (
    <details className="group rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-sm hover:border-white/20 transition overflow-hidden">
      ...
      {/* Brand accent divider (orange) */}
      <div className="h-px w-full bg-gradient-to-r from-orange-500/40 via-amber-500/30 to-orange-400/40" />
...
function BG(){
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      <div className="absolute -top-1/3 left-1/2 -translate-x-1/2 h-[80vh] w-[120vw] rounded-full bg-gradient-to-br from-orange-500/15 via-amber-500/10 to-orange-600/15 blur-3xl"/>
      <div className="absolute bottom-[-30vh] right-[-10vw] h-[60vh] w-[60vw] rounded-full bg-gradient-to-br from-amber-500/10 via-orange-500/10 to-amber-600/10 blur-3xl"/>
    </div>
  );
}
